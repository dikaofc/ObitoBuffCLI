/**
 * Obitobuff account standing ("Access Level") — the per-account policy layer
 ...
 * are scarce, and `docs/obitobuff-signup-gate.md` is the record of them not
 ...
 *
 * `ObitobuffAccessTier` (full / limited) is a REGION property, resolved per
 * request from the caller's IP country. `ObitobuffTrustLevel` is an ACCOUNT
 ...
 * naming the remedy. `docs/obitobuff-abuse-detection.md` records what it cost
 ...
 * `OBITOBUFF_TRUST_LEVEL_LABELS` is the one bridge between the two.
 */

import type { ObitobuffAccessTier } from './obitobuff-models'
...
export type ObitobuffTrustLevel = (typeof OBITOBUFF_TRUST_LEVELS)[number]
...
export const OBITOBUFF_TRUST_MIN_LEVEL: ObitobuffTrustLevel = 'new'
 ...
 */
export const OBITOBUFF_TRUST_FALLBACK_LEVEL: ObitobuffTrustLevel = 'established'

export function isAtLeastTrustLevel(
  level: ObitobuffTrustLevel,
  minimum: ObitobuffTrustLevel,
): boolean {
  return (
    OBITOBUFF_TRUST_LEVELS.indexOf(level) >=
    OBITOBUFF_TRUST_LEVELS.indexOf(minimum)
  )
}

function lowerOf(
  a: ObitobuffTrustLevel,
  b: ObitobuffTrustLevel,
): ObitobuffTrustLevel {
 ...
 *  their character. */
export const OBITOBUFF_TRUST_LEVEL_LABELS: Record<ObitobuffTrustLevel, string> = {
  new: 'Getting started',
  verified: 'Verified',
...
export const OBITOBUFF_TRUST_LEVEL_BLURBS: Record<ObitobuffTrustLevel, string> = {
  ...
  established:
    'You are an established Obitobuff user with generous limits on messages, spend and premium sessions.',
 ...
 */
export interface ObitobuffTrustLimits {
  /**
 ...
 *
 * **Browser sessions per day** (`OBITOBUFF_WEB_STANDARD_SESSION_LIMIT`) and
 * **concurrent Desktop tabs** (`OBITOBUFF_DESKTOP_SESSION_LIMITS.unlimited`) are
 ...
 */
export const OBITOBUFF_TRUST_LIMITS: Record<
  ObitobuffAccessTier,
  Record<ObitobuffTrustLevel, ObitobuffTrustLimits>
> = {
  full: {
    new: {
...
export function obitobuffTrustLimits(
  accessTier: ObitobuffAccessTier,
  level: ObitobuffTrustLevel,
): ObitobuffTrustLimits {
  return OBITOBUFF_TRUST_LIMITS[accessTier][level]
}
 ...
 * account created before `docs/obitobuff-signup-gate.md` shipped has null
 ...
 */
export interface ObitobuffTrustSignals {
  /** `user.created_at`. */
   ...
   *  (`obitobuff_daily_usage`). Cheap history that cannot be bought. */
  activeDays: number
...
export interface ObitobuffTrustFactor {
  id: string
  label: string
  points: number
}
...
export interface ObitobuffTrustNextStep {
  id: string
  label: string
  detail: string
  points: number
  /** Where the UI should send them. Relative to the obitobuff web app. */
  href?: string
}

export interface ObitobuffTrustAssessment {
  level: ObitobuffTrustLevel
  ...
  uncappedLevel: ObitobuffTrustLevel
  /** Which cap bound, if any. */
  cappedBy: string | null
  factors: ObitobuffTrustFactor[]
  nextSteps: ObitobuffTrustNextStep[]
}
 ...
 * (docs/obitobuff-bounties.md). A qualified referral requires the REFERRED
 ...
 * (docs/obitobuff-abuse-referral-farming.md). An operator who can produce ten
 * qualified referrals has already cleared a higher bar than this scorer sets.
 */
export const OBITOBUFF_TRUST_EARNED = {
  BOUNTY_POINTS: 5,
  BOUNTY_CAP: 6,
  REFERRAL_POINTS: 3,
  REFERRAL_CAP: 10,
...
const MAX_REFERRAL_POINTS =
  OBITOBUFF_TRUST_EARNED.REFERRAL_POINTS * OBITOBUFF_TRUST_EARNED.REFERRAL_CAP

/** Minimum score for each level. `new` is the floor and needs no entry. */
export const OBITOBUFF_TRUST_THRESHOLDS: Record<
  Exclude<ObitobuffTrustLevel, 'new'>,
  number
> = {
  verified: 25,
  established: 50,
...
function levelForScore(score: number): ObitobuffTrustLevel {
  if (score >= OBITOBUFF_TRUST_THRESHOLDS.core) return 'core'
  if (score >= OBITOBUFF_TRUST_THRESHOLDS.established) return 'established'
  if (score >= OBITOBUFF_TRUST_THRESHOLDS.verified) return 'verified'
  return 'new'
}
 ...
 */
export function assessObitobuffTrust(
  signals: ObitobuffTrustSignals,
  now: Date = new Date(),
): ObitobuffTrustAssessment {
  const factors: ObitobuffTrustFactor[] = []
  const nextSteps: ObitobuffTrustNextStep[] = []

  const add = (id: string, label: string, points: number) => {
    if (points === 0) return
    factors.push({ id, label, points })
  }
  const step = (s: ObitobuffTrustNextStep) => nextSteps.push(s)
  ...
  if (githubAge === null) {
    ...
    if (signals.githubTwoFactorEnabled) {
      ...
      step({
        ...
        detail:
          'Worth 5 points, and it protects the account your Obitobuff limits now depend on.',
        points: 5,
  ...
  if (accountAge !== null) {
    if (accountAge >= 90 * DAY_MS) {
      add('account_age', 'Obitobuff account over 90 days old', 15)
    } else if (accountAge >= 30 * DAY_MS) {
      add('account_age', 'Obitobuff account over 30 days old', 10)
    } else if (accountAge >= 7 * DAY_MS) {
      add('account_age', 'Obitobuff account over 7 days old', 5)
    }
  }

  if (signals.activeDays >= 30) {
    add('active_days', 'Used Obitobuff on 30+ days', 10)
  } else if (signals.activeDays >= 7) {
    add('active_days', 'Used Obitobuff on 7+ days', 5)
  }
  ...
  // a half.
  const bountyPoints =
    Math.min(signals.approvedBounties, OBITOBUFF_TRUST_EARNED.BOUNTY_CAP) *
  ...
  if (bountyPoints < MAX_BOUNTY_POINTS) {
    step({
      ...
      detail: `Approved bounties are worth ${OBITOBUFF_TRUST_EARNED.BOUNTY_POINTS} points each, up to ${MAX_BOUNTY_POINTS}. They are reviewed, they work from any country, and they pay session grants on top.`,
  ...
  const referralPoints =
    Math.min(signals.qualifiedReferrals, OBITOBUFF_TRUST_EARNED.REFERRAL_CAP) *
  ...
  if (referralPoints < MAX_REFERRAL_POINTS) {
    step({
      ...
      detail: `Each friend who signs up with a real GitHub account and uses Obitobuff is worth ${OBITOBUFF_TRUST_EARNED.REFERRAL_POINTS} points, up to ${MAX_REFERRAL_POINTS} — plus the referral rewards themselves.`,
  ...
  if (signals.hasPaid) {
    add('paid', 'Supported Obitobuff with a purchase', 25)
  }
  ...
  const cap = (limit: ObitobuffTrustLevel, reason: string) => {
 ...
 */
export interface ObitobuffStandingInfo {
  level: ObitobuffTrustLevel
  label: string
  blurb: string
  score: number
  ...
  nextLevel: ObitobuffTrustLevel | null
  cappedBy: string | null
  cappedReason: string | null
  factors: ObitobuffTrustFactor[]
  nextSteps: ObitobuffTrustNextStep[]
  accessTier: ObitobuffAccessTier
  limits: ObitobuffTrustLimits
}

export function toObitobuffStandingInfo(
  assessment: ObitobuffTrustAssessment,
  accessTier: ObitobuffAccessTier,
): ObitobuffStandingInfo {
  const index = OBITOBUFF_TRUST_LEVELS.indexOf(assessment.level)
  const nextLevel = OBITOBUFF_TRUST_LEVELS[index + 1] ?? null
  return {
    level: assessment.level,
    label: OBITOBUFF_TRUST_LEVEL_LABELS[assessment.level],
    blurb: OBITOBUFF_TRUST_LEVEL_BLURBS[assessment.level],
    score: assessment.score,
    nextLevel,
    nextLevelAt:
      nextLevel && nextLevel !== 'new'
        ? OBITOBUFF_TRUST_THRESHOLDS[nextLevel]
        : null,
    ...
    accessTier,
    limits: obitobuffTrustLimits(accessTier, assessment.level),
  }
}
