 ...
 *
 * This list started in `obitobuff-desktop/src/shared/types.ts`, where it drives
 * the Claude Code and Codex effort pickers. It lives here now so the Obitobuff
 * model catalog and Desktop speak the same words: Desktop re-exports these,
 * so a value persisted on a Desktop thread and a value on a Obitobuff catalog
