...
import {
  OBITOBUFF_GEMINI_PRO_AGENT_IDS,
  OBITOBUFF_GEMINI_THINKER_AGENT_ID,
} from './obitobuff-gemini-thinker'
import {
...
/**
 * The root agent family Obitobuff Desktop's hosted (codebuff) harness runs every
 * thread turn under (see obitobuff-desktop thread-agent.ts). Unlike the CLI — which
 ...
 * OBITOBUFF_ROOT_AGENT_IDS below and carry the "You are Buffy" CLI marker in their
 * system prompts so they pass requestHasObitobuffSystemMarker.
 */
export const OBITOBUFF_DESKTOP_THREAD_AGENT_ID = 'obitobuff-desktop-thread'
 ...
 */
export const OBITOBUFF_DESKTOP_THREAD_V3_SUFFIX = 'v3'

export function getObitobuffDesktopThreadAgentId(
  executionMode: 'local' | 'worktree',
  agentGeneration: 'base2' | 'base3' = 'base2',
): string {
  const base = `${OBITOBUFF_DESKTOP_THREAD_AGENT_ID}-${executionMode}`
  return agentGeneration === 'base3'
    ? `${base}-${OBITOBUFF_DESKTOP_THREAD_V3_SUFFIX}`
    : base
}
 ...
 */
export const OBITOBUFF_DESKTOP_THREAD_AGENT_IDS = [
  OBITOBUFF_DESKTOP_THREAD_AGENT_ID,
  getObitobuffDesktopThreadAgentId('local'),
  getObitobuffDesktopThreadAgentId('worktree'),
  getObitobuffDesktopThreadAgentId('local', 'base3'),
  getObitobuffDesktopThreadAgentId('worktree', 'base3'),
] as const

/**
 * The Obitobuff Web and Cloud roots that run the base3 single-loop harness
 ...
 * resolving, and the OBITOBUFF_BASE3_HARNESS_DISABLED kill switch routes new
 * turns back to them without a deploy.
 *
 * Every key here must also be a key of the web bundle's
 * OBITOBUFF_MODEL_TO_AGENT_ID (obitobuff_bundled_agents.ts asserts it): a model
 ...
 */
export const OBITOBUFF_WEB_BASE3_AGENT_ID_BY_MODEL: Record<string, string> = {
  [OBITOBUFF_DEEPSEEK_V4_PRO_MODEL_ID]: 'base3-free-deepseek',
  [OBITOBUFF_DEEPSEEK_V4_FLASH_MODEL_ID]: 'base3-free-deepseek-flash',
  [OBITOBUFF_MIMO_V25_MODEL_ID]: 'base3-free-mimo',
  [OBITOBUFF_MINIMAX_M3_MODEL_ID]: 'base3-free-minimax-m3',
  [OBITOBUFF_GPT_5_6_LUNA_MODEL_ID]: 'base3-free-luna',
  [OBITOBUFF_GLM_V52_MODEL_ID]: 'base3-free-glm',
  [OBITOBUFF_KIMI_K3_ECO_MODEL_ID]: 'base3-free-kimi-k3-eco',
  [OBITOBUFF_MUSE_SPARK_12_CONTRIBUTOR_MODEL_ID]: 'base3-free-muse-spark',
}

/**
 * The Obitobuff CLI roots that run the base3 single-loop harness (agents/
 ...
 * which Web never surfaces. `obitobuff_bundled_agents.test.ts` asserts the Web
 * map covers exactly the Web base2 models, so a CLI-only model added there
 * would fail that parity check for the wrong reason.
 */
export const OBITOBUFF_CLI_BASE3_AGENT_ID_BY_MODEL: Record<string, string> = {
  [OBITOBUFF_DEEPSEEK_V4_PRO_MODEL_ID]: 'base3-free-deepseek',
  [OBITOBUFF_DEEPSEEK_V4_FLASH_MODEL_ID]: 'base3-free-deepseek-flash',
  [OBITOBUFF_MIMO_V25_MODEL_ID]: 'base3-free-mimo',
  [OBITOBUFF_MINIMAX_M3_MODEL_ID]: 'base3-free-minimax-m3',
  [OBITOBUFF_GPT_5_6_LUNA_MODEL_ID]: 'base3-free-luna',
  [OBITOBUFF_GLM_V52_MODEL_ID]: 'base3-free-glm',
  [OBITOBUFF_FABLE_5_MODEL_ID]: 'base3-free-fable',
}

/** Every base3 root id, whichever surface registered it. */
export const OBITOBUFF_BASE3_AGENT_IDS: ReadonlySet<string> = new Set([
  ...Object.values(OBITOBUFF_WEB_BASE3_AGENT_ID_BY_MODEL),
  ...Object.values(OBITOBUFF_CLI_BASE3_AGENT_ID_BY_MODEL),
])

/**
 * The Obitobuff Cloud custom-stack planner roots, and the models they are pinned
...
export const CLOUD_PLANNER_MODEL_ID = OBITOBUFF_DEEPSEEK_V4_FLASH_MODEL_ID
export const CLOUD_PLANNER_LIMITED_AGENT_ID = 'base2-free-cloud-planner-limited'
export const CLOUD_PLANNER_LIMITED_MODEL_ID = LIMITED_OBITOBUFF_MODEL_ID

/**
 * The model the build runs on after "Start building".
 *
 * DeepSeek V4 Flash since 2026-08-01 (was V4 Pro), for the same reason
 * DEFAULT_OBITOBUFF_WEB_MODEL_ID is: the V4-Flash-0731 GA build is the strongest
 ...
 */
export const CLOUD_BUILD_MODEL_ID = OBITOBUFF_DEEPSEEK_V4_FLASH_MODEL_ID
...
export function isCloudBuildModelId(model: string | null | undefined): boolean {
  return model === CLOUD_BUILD_MODEL_ID || model === FALLBACK_OBITOBUFF_MODEL_ID
}
...
/**
 * Root-orchestrator agent IDs counted as "a obitobuff session" for abuse
 ...
 */
export const OBITOBUFF_ROOT_AGENT_IDS = [
  'base2-free',
  'base2-free-deepseek',
  ...
  // Obitobuff Web only (Meta Muse Spark 1.2 Contributor). Listed here like every
  ...
  // mid-run.
  'base2-free-fable',
  // Obitobuff Cloud custom-stack planner variants. They spawn context-pruner, so
  ...
  // "You are Buffy" marker so they also pass requestHasObitobuffSystemMarker.
  'base2-free-cloud-planner',
  'base2-free-cloud-planner-limited',
  // Obitobuff Web and Cloud base3 roots (single-loop harness). Listed
  // individually rather than spread from
  // OBITOBUFF_WEB_BASE3_AGENT_ID_BY_MODEL so the ids stay greppable; a test in
  ...
  // Obitobuff CLI base3 roots. Every other id it needs is already above,
  // shared with Web; Fable is the one model the CLI offers and Web does not.
  'base3-free-fable',
  ...OBITOBUFF_DESKTOP_THREAD_AGENT_IDS,
] as const
const OBITOBUFF_ROOT_AGENT_ID_SET: ReadonlySet<string> = new Set(
  OBITOBUFF_ROOT_AGENT_IDS,
)

export const OBITOBUFF_ROOT_AGENT_ID_BY_MODEL: Record<string, string> = {
  [OBITOBUFF_MIMO_V25_MODEL_ID]: 'base2-free-mimo',
  [OBITOBUFF_MINIMAX_M3_MODEL_ID]: 'base2-free-minimax-m3',
  [OBITOBUFF_GPT_5_6_LUNA_MODEL_ID]: 'base2-free-luna',
  [OBITOBUFF_DEEPSEEK_V4_PRO_MODEL_ID]: 'base2-free-deepseek',
  [OBITOBUFF_DEEPSEEK_V4_FLASH_MODEL_ID]: 'base2-free-deepseek-flash',
  [OBITOBUFF_GLM_V52_MODEL_ID]: 'base2-free-glm',
  [OBITOBUFF_KIMI_K3_ECO_MODEL_ID]: 'base2-free-kimi-k3-eco',
  [OBITOBUFF_FABLE_5_MODEL_ID]: 'base2-free-fable',
  [OBITOBUFF_MUSE_SPARK_12_CONTRIBUTOR_MODEL_ID]: 'base2-free-muse-spark',
}

/**
 * The reviewer each obitobuff root spawns, keyed by the root's model.
 *
 ...
 * reviewer, which is itself a obitobuff session model, so the fallback 403s for
 ...
 */
export const OBITOBUFF_REVIEWER_AGENT_ID_BY_MODEL: Record<string, string> = {
  [OBITOBUFF_MIMO_V25_MODEL_ID]: 'code-reviewer-mimo',
  [OBITOBUFF_MINIMAX_M3_MODEL_ID]: 'code-reviewer-minimax-m3',
  [OBITOBUFF_GPT_5_6_LUNA_MODEL_ID]: 'code-reviewer-luna',
  ...
  [OBITOBUFF_GLM_V52_MODEL_ID]: 'code-reviewer-glm',
  [OBITOBUFF_FABLE_5_MODEL_ID]: 'code-reviewer-fable',
}

const OBITOBUFF_DESKTOP_MODELS = new Set([
  OBITOBUFF_MINIMAX_M3_MODEL_ID,
  OBITOBUFF_GPT_5_6_LUNA_MODEL_ID,
  OBITOBUFF_DEEPSEEK_V4_PRO_MODEL_ID,
  OBITOBUFF_DEEPSEEK_V4_FLASH_MODEL_ID,
...
export function getObitobuffRootAgentIdForModel(model: string): string {
  return OBITOBUFF_ROOT_AGENT_ID_BY_MODEL[model] ?? 'base2-free'
}

/**
 * The base3 root the Obitobuff CLI runs for a selected model.
 *
 * Falls back to the model's own base2 root, not to some other model's base3
 * root, for the reason resolveObitobuffAgentId does the same on Web: running the
 ...
 */
export function getObitobuffBase3RootAgentIdForModel(model: string): string {
  return (
    OBITOBUFF_CLI_BASE3_AGENT_ID_BY_MODEL[model] ??
...
export const FREE_MODE_AGENT_MODELS: Record<string, Set<string>> = {
  ...
  // one model id.
  'base2-free-deepseek': new Set([OBITOBUFF_DEEPSEEK_V4_PRO_MODEL_ID]),
  'base2-free-deepseek-flash': new Set([OBITOBUFF_DEEPSEEK_V4_FLASH_MODEL_ID]),
  'base2-free-mimo': new Set([OBITOBUFF_MIMO_V25_MODEL_ID]),
  'base2-free-minimax-m3': new Set([OBITOBUFF_MINIMAX_M3_MODEL_ID]),
  'base2-free-luna': new Set([OBITOBUFF_GPT_5_6_LUNA_MODEL_ID]),
  'base2-free-glm': new Set([OBITOBUFF_GLM_V52_MODEL_ID]),
  'base2-free-kimi-k3-eco': new Set([OBITOBUFF_KIMI_K3_ECO_MODEL_ID]),
  ...
  'base2-free-fable': new Set([OBITOBUFF_FABLE_5_MODEL_ID]),
  // Obitobuff Cloud custom-stack planner (obitobuff_bundled_agents.ts). One
  ...
  'base2-free-cloud-planner-limited': new Set([LIMITED_OBITOBUFF_MODEL_ID]),
  ...
  ...Object.fromEntries(
    [
      ...Object.entries(OBITOBUFF_WEB_BASE3_AGENT_ID_BY_MODEL),
      ...Object.entries(OBITOBUFF_CLI_BASE3_AGENT_ID_BY_MODEL),
    ].map(([model, agentId]) => [agentId, new Set([model])]),
  ),

  // Every Obitobuff Desktop hosted root variant allows the full desktop picker
  ...
  [OBITOBUFF_DESKTOP_THREAD_AGENT_ID]: OBITOBUFF_DESKTOP_MODELS,
  [getObitobuffDesktopThreadAgentId('local')]: OBITOBUFF_DESKTOP_MODELS,
  [getObitobuffDesktopThreadAgentId('worktree')]: OBITOBUFF_DESKTOP_MODELS,
  [getObitobuffDesktopThreadAgentId('local', 'base3')]: OBITOBUFF_DESKTOP_MODELS,
  [getObitobuffDesktopThreadAgentId('worktree', 'base3')]:
    OBITOBUFF_DESKTOP_MODELS,
  ...
  'tmux-cli': new Set([OBITOBUFF_DEEPSEEK_V4_FLASH_MODEL_ID]),

  // Code reviewer for free mode
  'code-reviewer-minimax-m3': new Set([OBITOBUFF_MINIMAX_M3_MODEL_ID]),
  'code-reviewer-luna': new Set([OBITOBUFF_GPT_5_6_LUNA_MODEL_ID]),
  'code-reviewer-deepseek': new Set([OBITOBUFF_DEEPSEEK_V4_PRO_MODEL_ID]),
  'code-reviewer-deepseek-flash': new Set([
    OBITOBUFF_DEEPSEEK_V4_FLASH_MODEL_ID,
  ]),
  'code-reviewer-mimo': new Set([OBITOBUFF_MIMO_V25_MODEL_ID]),
  'code-reviewer-glm': new Set([OBITOBUFF_GLM_V52_MODEL_ID]),
  'code-reviewer-fable': new Set([OBITOBUFF_FABLE_5_MODEL_ID]),
  // Wire compatibility only — NOT a obitobuff agent. `code-reviewer-lite` now
  // belongs to Codebuff's paid lite mode and is spawned by no obitobuff root and
  // shipped in no obitobuff bundle. Released clients from before the
  ...
  //
  // Never add lite's model (GPT-5.6 Luna) here. Obitobuff now offers that model
  // too, but it reaches it through its OWN agents — base2-free-luna and
  // code-reviewer-luna — which carry Obitobuff's pinned OpenAI routing and
  ...
  [OBITOBUFF_GEMINI_THINKER_AGENT_ID]: new Set([OBITOBUFF_GEMINI_PRO_MODEL_ID]),
}
...
export function isObitobuffRootAgent(fullAgentId: string): boolean {
  const { publisherId, agentId } = parseAgentId(fullAgentId)
  if (!agentId) return false
  if (publisherId && publisherId !== 'codebuff') return false
  return OBITOBUFF_ROOT_AGENT_ID_SET.has(agentId)
}

/**
 * The opening sentence of every first-party obitobuff root system prompt, one
 * per prompt family. A free-mode root request must open with one of these
 * verbatim (see hasObitobuffRootSystemPromptOpening).
 *
 * These are copies, not imports: the definitions live in three packages the web
 * API cannot pull in (agents/base2/base2.ts,
 * obitobuff/web/convex/.../obitobuff_bundled_agents.ts,
 * obitobuff-desktop/.../thread-agent.ts). `free-agents.test.ts` reads those
 ...
 */
export const OBITOBUFF_ROOT_SYSTEM_PROMPT_OPENINGS = [
  ...
  // obitobuff_bundled_agents.ts CLOUD_PLANNER_SYSTEM_PROMPT — planner roots.
  'You are Buffy, the Obitobuff Cloud project planner.',
  ...
  // commit. Measured at 4 of 4,979 obitobuff launches over the 7d to 2026-07-31
...
/**
 * True when `text` opens with one of the canonical obitobuff root prompts.
 *
 ...
 * the public obitobuff2api proxy passed it by prepending
 ...
 * caller has to actually send the obitobuff coding-agent identity as the first
 ...
 */
export function hasObitobuffRootSystemPromptOpening(text: string): boolean {
  const trimmed = text.trimStart()
  return OBITOBUFF_ROOT_SYSTEM_PROMPT_OPENINGS.some((opening) =>
    trimmed.startsWith(opening),
  )
}

export function isObitobuffGeminiThinkerAgent(fullAgentId: string): boolean {
  const { publisherId, agentId } = parseAgentId(fullAgentId)
  if (!agentId) return false
  if (publisherId && publisherId !== 'codebuff') return false
  return agentId === OBITOBUFF_GEMINI_THINKER_AGENT_ID
}
 ...
 */
export function isObitobuffGeminiProAgent(fullAgentId: string): boolean {
  const { publisherId, agentId } = parseAgentId(fullAgentId)
  if (!agentId) return false
  if (publisherId && publisherId !== 'codebuff') return false
  return OBITOBUFF_GEMINI_PRO_AGENT_IDS.has(agentId)
}
