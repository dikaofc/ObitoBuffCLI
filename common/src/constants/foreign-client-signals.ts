...
/**
 * Where a free-mode request goes when it did not come from a obitobuff client.
 *
 ...
 */
export const OBITOBUFF_DOWNGRADE_MODEL_ID = 'inclusionai/ling-3.0-tiny:free'
 ...
 * `customToolDefinitions`. Obitobuff Desktop's autorun agent
 * (obitobuff-desktop/src/server/services/autorun.ts) offers exactly `decide` and
 ...
 */
export const OBITOBUFF_CUSTOM_TOOL_NAMES = ['decide'] as const
 ...
 */
export const OBITOBUFF_SIGNATURE_TOOL_NAMES: ReadonlySet<string> = new Set([
  ...(toolNames as readonly string[]).filter(
    (name) => !GENERIC_TOOL_NAMES.has(name),
  ),
  ...OBITOBUFF_CUSTOM_TOOL_NAMES,
])
...
/**
 * Whether a free-mode request came from something other than a obitobuff client.
 *
 ...
 */
export function detectForeignObitobuffClient(
  ...
  if (offered.length > 0) {
    const hasSignatureTool = offered.some((name) =>
      OBITOBUFF_SIGNATURE_TOOL_NAMES.has(name),
    )
    return {
  ...
  //   obitobuff-desktop-thread-local 0.018%   2,400 reqs      11 users
  //
  ...
  // canonical root system prompt at position 0 — `requestHasObitobuffSystemMarker`
 ...
 * endpoint is a terms violation, not a grey area: Obitobuff funds free
 ...
 *    `detectForeignObitobuffClient`. Catching the 417 genuine proxies inside
...
export function resolveForeignClientDowngrade(params: {
  ...
  const verdict = detectForeignObitobuffClient(body, isRootAgent)
  ...
  return {
    ...
    downgradeTo:
      ...
      body.model !== OBITOBUFF_DOWNGRADE_MODEL_ID
