...
describe('Gravity first-message CAPI', () => {
  test('sanitizes browser attribution in one shared boundary', () => {
    expect(
      ...
      event_source_url: 'https://obitobuff.com/chat',
      client_context: { timezone: 'America/Los_Angeles' },
    })
    expect(isObitobuffGravitySurface('desktop')).toBe(true)
    expect(isObitobuffGravitySurface('preexisting')).toBe(false)
  })
  ...
  test('builds a stable, attributed payload with authoritative identity', () => {
    expect(
      buildGravityFirstMessagePayload({
        ...
        eventTime: 123,
        gravity: {
          event_source_url: 'https://obitobuff.com/cloud/project/example',
          user_data: {
            click_id: 'click-1',
      ...
      data: [
        {
          ...
          event_time: 123,
          event_id: 'obitobuff-first-message-user-123',
  ...
  test('uses the same event id across surfaces for Gravity deduplication', () => {
    expect(gravityFirstMessageEventId('user-123')).toBe(
      'obitobuff-first-message-user-123',
    )
  })
