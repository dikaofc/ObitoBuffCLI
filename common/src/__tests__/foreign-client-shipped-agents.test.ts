...
import { OBITOBUFF_ROOT_AGENT_IDS } from '../constants/free-agents'
import {
 ...
 * obitobuff agent the detector flags is served a different model in production
 ...
 *  - `obitobuff-desktop-autorun` offers only the custom tool `decide`, so it had
...
const EXCLUDED: Array<{ path: string; mustContain: string; why: string }> = [
  {
    path: 'obitobuff/e2e/agent/obitobuff-tester.ts',
...
describe('no shipped obitobuff agent is flagged as a foreign client', () => {
  ...
  test.each([
    ...
    ['desktop autorun', 'obitobuff-desktop/src/server/services/autorun.ts'],
  ...
  test.each([...OBITOBUFF_CUSTOM_TOOL_NAMES])(
    ...
    (name) => {
      ...
      expect(
        detectForeignObitobuffClient({ tools: asToolSchemas([name]) }).signal,
      ).toBeNull()
    },
  )
  ...
  test('custom tools appended to a real toolset stay cleared', () => {
    ...
    // document tools (obitobuff/web/src/server/chat/agent.ts). The additions are
