...
import {
  addDaysToDateKey,
  calculateObitobuffStreak,
  getObitobuffDailyStreakRewardPool,
  getObitobuffStreakGlmBonusUnits,
  getObitobuffUsageDateKey,
...
describe('obitobuff streak helpers', () => {
  test('formats usage dates in the Obitobuff reset timezone', () => {
    expect(getObitobuffUsageDateKey(new Date('2026-05-27T06:30:00.000Z'))).toBe(
      '2026-05-26',
    )
    expect(getObitobuffUsageDateKey(new Date('2026-05-27T08:30:00.000Z'))).toBe(
      '2026-05-27',
    )
  })
...
describe('obitobuff streak rewards', () => {
  ...
  test('no daily reward below seven days or before today is used', () => {
    ...
    // Obitobuff today, so no bonus is earned yet.
    expect(
      getObitobuffDailyStreakRewardPool({
        streak: 7,
        todayUsed: false,
