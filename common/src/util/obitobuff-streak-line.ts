import { OBITOBUFF_STREAK_REWARDS_ENABLED } from '../constants/obitobuff-models'
import {
  getObitobuffStreakGlmWeeklyUnits,
  isObitobuffStreakGlmBonusActive,
} from './obitobuff-streak'

/** Days in a streak "week" — the milestone the progress dots fill toward. */
export const OBITOBUFF_STREAK_WEEK = 7

export interface ObitobuffStreakLine {
   ...
   *  OBITOBUFF_STREAK_WEEK, carried so a renderer that can't import this module
...
export interface ObitobuffStreakDotChars {
  filled: string
  empty: string
}
...
export function getObitobuffStreakLine(
  streak: number,
  chars: ObitobuffStreakDotChars = { filled: '●', empty: '○' },
  ...
  const filled = Math.min(streak, OBITOBUFF_STREAK_WEEK)
  const beyond = streak > OBITOBUFF_STREAK_WEEK
  const dots =
    chars.filled.repeat(filled) +
    chars.empty.repeat(OBITOBUFF_STREAK_WEEK - filled) +
    (beyond ? '+' : '')
  ...
  return {
    ...
    dots,
    progress: { filled, total: OBITOBUFF_STREAK_WEEK, beyond },
  }
}
 ...
 */
export function getObitobuffStreakBonusNote(params: {
  streak: number
  ...
  const includesGlm =
    params.accessTier === 'full' && isObitobuffStreakGlmBonusActive()
  ...
  const glmDaily = Math.max(1, getObitobuffStreakGlmWeeklyUnits(params.streak))
  ...
  if (params.streak < OBITOBUFF_STREAK_WEEK) {
    const remaining = OBITOBUFF_STREAK_WEEK - params.streak
