...
export const OBITOBUFF_GRAVITY_SURFACES = [
  'cli',
  'desktop',
  'web',
  'cloud',
...
export type ObitobuffGravitySurface = (typeof OBITOBUFF_GRAVITY_SURFACES)[number]
export type ObitobuffServiceGravitySurface = Extract<
  ObitobuffGravitySurface,
  'web' | 'cloud' | 'chat'
>

export function isObitobuffGravitySurface(
  value: unknown,
): value is ObitobuffGravitySurface {
  return OBITOBUFF_GRAVITY_SURFACES.includes(value as ObitobuffGravitySurface)
}

export function isObitobuffServiceGravitySurface(
  value: unknown,
): value is ObitobuffServiceGravitySurface {
...
export function gravityFirstMessageEventId(userId: string): string {
  return `obitobuff-first-message-${userId}`
}

export function buildGravityFirstMessagePayload(params: {
  userId: string
  email?: string | null
  surface: ObitobuffGravitySurface
    ...
    data: [
      {
        ...
        ...(params.gravity?.client_context
          ...
          : {}),
        custom_data: {
          content_name: 'Obitobuff first message',
...
export async function sendGravityFirstMessageConversion(params: {
  ...
  const response = await fetchImpl(GRAVITY_CAPI_ENDPOINT, {
    ...
    headers: {
      ...
      'User-Agent': 'obitobuff/1.0 (gravity-capi)',
    },
