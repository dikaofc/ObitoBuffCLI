...
/**
 * Obitobuff CLI release script.
 *
 * Triggers the obitobuff-release.yml GitHub Actions workflow
 * to build, publish, and release the Obitobuff CLI to npm.
 *
 * Usage:
 *   bun obitobuff/cli/release.ts [patch|minor|major] [--ref <commit-sha>]
 *
 * Requires:
...
async function triggerWorkflow(versionType: string, checkoutRef: string) {
  ...
  try {
    ...
    const triggerCmd = `curl -s -w "HTTP Status: %{http_code}" -X POST \
      ...
      https://api.github.com/repos/CodebuffAI/obitobuff-private/actions/workflows/obitobuff-release.yml/dispatches \
      -d '${payload}'`
    ...
    if (response.includes('workflow_dispatch')) {
      ...
      log(
        'Please manually trigger the workflow at: https://github.com/CodebuffAI/obitobuff-private/actions/workflows/obitobuff-release.yml',
      )
    } else {
      log('🎉 Obitobuff release workflow triggered!')
    }
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err)
    log(`⚠️  Failed to trigger workflow automatically: ${message}`)
    log(
      'You may need to trigger it manually at: https://github.com/CodebuffAI/obitobuff-private/actions/workflows/obitobuff-release.yml',
    )
  }
}

async function main() {
  log('🚀 Initiating Obitobuff release...')
  ...
  log('')
  log(
    'Monitor progress at: https://github.com/CodebuffAI/obitobuff-private/actions/workflows/obitobuff-release.yml',
  )
}
