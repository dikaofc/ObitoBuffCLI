...
export class ObitobuffSession {
  ...
  /**
   * Start a obitobuff binary in a tmux session.
  ...
  static async start(
  ...
  ): Promise<ObitobuffSession> {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'obitobuff-e2e-'))

    // Create a minimal project so obitobuff has something to work with
    fs.writeFileSync(
    ...
    return new ObitobuffSession(sessionName, tmpDir)
  }
  ...
  /** Send text input to the obitobuff CLI (presses Enter by default). */
  async send(
    text: string,
  ...
  async waitForBootSignal(timeoutMs = 30_000): Promise<string> {
    ...
    while (Date.now() - start < timeoutMs) {
      const output = await this.capture()
      if (OBITOBUFF_BOOT_SIGNALS.some((signal) => output.includes(signal))) {
        return output
      }
    ...
    throw new Error(
      `Timed out after ${timeoutMs}ms waiting for a boot signal ` +
        `(checked ${OBITOBUFF_BOOT_SIGNALS.length} patterns).\n` +
        `Last output:\n${finalOutput}`,
    )
  }
