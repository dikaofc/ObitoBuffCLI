...
/**
 * Commands that should be REMOVED in Obitobuff.
 * These are stripped at build time via the OBITOBUFF_REMOVED_COMMAND_IDS set
...
/**
 * Commands that should be KEPT in Obitobuff.
...
describe.skip('Obitobuff: Slash Commands', () => {
  ...
  test(
    ...
    async () => {
      ...
      session = await ObitobuffSession.start(binary, { waitSeconds: 5, height: SESSION_HEIGHT })
  ...
  test(
    ...
    async () => {
      ...
      // Mode commands should not exist in Obitobuff
