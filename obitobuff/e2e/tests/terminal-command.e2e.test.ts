/**
 * E2E test that verifies Obitobuff can run terminal commands.
 *
 * Starts Obitobuff in tmux, sends a prompt asking it to run a shell command,
...
describe.skip('Obitobuff: Terminal Command', () => {
  ...
  test(
    ...
    async () => {
      ...
      session = await ObitobuffSession.start(binary, { waitSeconds: 5 })

      // Wait for the CLI to be fully ready before sending input
      await session.waitForReady()

      // Ask obitobuff to run a shell command whose output can only come from
