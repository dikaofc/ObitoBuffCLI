...
const gravitySurface = (agentTemplate: { id: string }): string => {
  if (agentTemplate.id === 'base-chat') return 'obitobuff_chat'
  // Obitobuff Web project agents are the `base2-free*` and `base3-free*`
...
const isServiceAccountSurface = (surface: string): boolean =>
  surface === 'obitobuff_chat' || surface === 'obitobuff_web'
...
export const handleGravityIndex = (async (params: {
  ...
  try {
    ...
    const input = {
      ...
      // Shared service-account surfaces (Obitobuff Web) authenticate the web API
      ...
      // (e.g. `obitobuff-chat-<userId>` or the project id), so forward it as the
