...
describe('gravity_index tool', () => {
  ...
  test('tags base-chat traffic with the obitobuff_chat surface', async () => {
    const spy = spyOn(webApi, 'callGravityIndexAPI').mockResolvedValue({
  ...
  // Both obitobuff Web root families, because the harness swap changed the id
  ...
  test.each(['base2-free-deepseek', 'base3-free-deepseek'])(
    'tags %s traffic with the obitobuff_web surface and forwards external_user_id',
    async (rootAgentId) => {
      const spy = spyOn(webApi, 'callGravityIndexAPI').mockResolvedValue({
        result: { search_id: 'search-1' },
      })
      ...
      expect(spy).toHaveBeenCalledWith(
        expect.objectContaining({
          input: expect.objectContaining({
            // Obitobuff Web runs under a shared service account, so the handler
