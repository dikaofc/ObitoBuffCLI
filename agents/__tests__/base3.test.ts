...
import { SUPPORTED_OBITOBUFF_MODELS } from '@codebuff/common/constants/obitobuff-models'
 ...
 * unfinished on a weak model (see docs/obitobuff-base3-harness.md). That is
 ...
 * (obitobuff_bundled_agents.test.ts); these are the CLI's, which ship compiled
...
describe('base3 CLI roots', () => {
  ...
  test('declares no reasoning, leaving the catalog the single authority', () => {
    ...
    // applyObitobuffReasoningDefaults unable to tell a model default apart from
  ...
  test('opens with a prompt the free-mode gate accepts', () => {
    ...
    for (const agent of CLI_ROOTS) {
      expect(hasObitobuffRootSystemPromptOpening(agent.systemPrompt!)).toBe(true)
    }
  })

  test('every Obitobuff root is pinned to the model its id is registered under', () => {
  ...
  test('ships a root for every model the picker offers', () => {
    for (const model of SUPPORTED_OBITOBUFF_MODELS) {
      const agentId = OBITOBUFF_CLI_BASE3_AGENT_ID_BY_MODEL[model.id]
  ...
  test('leaves the bare harness alone, so Desktop does not inherit CLI tools', () => {
    // obitobuff-desktop builds THREAD_AGENT_TOOLS by unioning
  ...
  test('brands Obitobuff roots as Obitobuff, and Codebuff roots as Codebuff', () => {
    expect(base3FreeDeepseek.systemPrompt).toContain('Obitobuff')
    expect(base3FreeDeepseek.systemPrompt).not.toContain('/usage')
    // Codebuff's paid modes explain credits; Obitobuff has none to explain.
