 ...
 * Obitobuff model the picker offers.
 *
 * Kept separate from `createBase3` rather than folded into it, because Obitobuff
 ...
 * obitobuff-desktop/.../thread-agent.ts unions that array with its own extras).
 ...
 * roots (docs/obitobuff-base3-harness.md):
 *
 * - The appendix is APPENDED. `hasObitobuffRootSystemPromptOpening` requires a
...
export function createBase3CliRoot(
  options: {
    ...
    /** Obitobuff branding and meta-information instead of Codebuff's. */
    isObitobuff?: boolean
  ...
  const { model = OPUS_MODEL, isObitobuff = false, noAskUser = false } = options
  ...
  const root: Omit<SecretAgentDefinition, 'id'> = {
    ...
    // which is how `obitobuff-desktop-autorun` shipped flagged.
    //
...
${buildCliAppendix({ isObitobuff, model, noAskUser })}`,
  }
...
${
  isObitobuff
    ? 'You are the AI agent behind Obitobuff, a tool where users can chat with you to code with AI for free. See obitobuff.com for more information about the product.'
    : [
