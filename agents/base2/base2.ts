...
import {
  ...
  OBITOBUFF_GEMINI_THINKER_INSTRUCTIONS_PROMPT,
  OBITOBUFF_GEMINI_THINKER_SYSTEM_INSTRUCTION,
} from '@codebuff/common/constants/obitobuff-gemini-thinker'
import { OBITOBUFF_REVIEWER_AGENT_ID_BY_MODEL } from '@codebuff/common/constants/free-agents'
import {
...
/**
 * Free mode runs MiniMax M3 (routed through the Fireworks AI API). New Obitobuff
...
const MODEL_BY_MODE = {
  ...
  fast: OPUS_MODEL,
  lite: LITE_MODEL,
  free: OBITOBUFF_MINIMAX_M3_MODEL_ID,
 ...
 * own reviewer on top of the shared ones; Obitobuff deliberately gets only the
 * free-tier map, so no free session can resolve to code-reviewer-lite even if a
 * obitobuff agent were pointed at lite's model. Anything unmapped falls back to
...
export function createBase2(
  ...
  //
  // Obitobuff gates it on the parent model: that set is a free-session admission
  // rule (see canObitobuffModelSpawnGeminiThinker and free-session/public-api),
  ...
  const hasGeminiThinker =
    isLite || (isObitobuff && canObitobuffModelSpawnGeminiThinker(model))
  ...
  return {
    ...
    spawnableAgents: buildArray(
      ...
      hasGeminiThinker && OBITOBUFF_GEMINI_THINKER_AGENT_ID,
      !isObitobuff && 'thinker-gpt',
      'context-pruner',
    ),

    systemPrompt: `You are Buffy, the strategic coding assistant. You are the AI agent behind the product, ${isObitobuff ? 'Obitobuff' : 'Codebuff'}, a tool where users can chat with you to code with AI${isObitobuff ? ' for free' : ''}.
...
- **Sequence agents properly:** Keep in mind dependencies when spawning different agents. Don't spawn agents in parallel that depend on each other.
  ${buildArray(
    ...
    hasGeminiThinker && OBITOBUFF_GEMINI_THINKER_SYSTEM_INSTRUCTION,
    isLite &&
...
# ${isObitobuff ? 'Obitobuff' : 'Codebuff'} Meta-information

You are running on the ${model} model.

${
  isObitobuff
    ? 'See obitobuff.com for more information about the product.'
    : [
...
`,
    ...
    // variables like `isObitobuff` are not in scope at runtime. Pick the right
...
${buildArray(
  ...
  `- ${THINKER_SPAWN_LIMIT}`,
  hasGeminiThinker && OBITOBUFF_GEMINI_THINKER_INSTRUCTIONS_PROMPT,
  (isDefault || isMax) &&
