...
async function triggerWorkflow(versionType) {
  ...
  try {
    ...
    const triggerCmd = `curl -s -w "HTTP Status: %{http_code}" -X POST \
      ...
      https://api.github.com/repos/CodebuffAI/obitobuff-private/actions/workflows/sdk-release.yml/dispatches \
    ...
    if (response.includes('workflow_dispatch')) {
      ...
      log(
        'Please manually trigger the workflow at: https://github.com/CodebuffAI/obitobuff-private/actions/workflows/sdk-release.yml',
      )
    } else {
    ...
    log(
      'You may need to trigger it manually at: https://github.com/CodebuffAI/obitobuff-private/actions/workflows/sdk-release.yml',
    )
  }
}
