import { AGENT_MODES, IS_LOCAL_MODE, IS_OBITOBUFF } from '../utils/constants'
...
// Generate mode commands from the AGENT_MODES constant (excluded in Obitobuff)
const MODE_COMMANDS: SlashCommand[] = IS_OBITOBUFF
  ? []
...
const OBITOBUFF_REMOVED_COMMAND_IDS = new Set([
  'ads:enable',
  'ads:disable',
  'usage',
  'subscribe',
...
const ALL_SLASH_COMMANDS: SlashCommand[] = [
  ...
  // },
  {
    id: 'feedback',
    label: 'feedback',
    description: IS_OBITOBUFF ? 'Share general feedback about Obitobuff' : 'Share general feedback about Codebuff',
  },
  {
    id: 'bash',
    label: 'bash',
...
// Local-mode model switcher (see commands/command-registry.ts). Shown only
// when running on configured providers, where /end-session is meaningless.
const LOCAL_ONLY_COMMANDS: SlashCommand[] = [
  {
    id: 'model',
    label: 'model',
    description: 'List configured models, or switch with /model <id>',
  },
]

export const SLASH_COMMANDS: SlashCommand[] = IS_LOCAL_MODE
  ? [
      ...ALL_SLASH_COMMANDS.filter(
        (cmd) =>
          !OBITOBUFF_REMOVED_COMMAND_IDS.has(cmd.id) &&
          cmd.id !== 'end-session',
      ),
      ...LOCAL_ONLY_COMMANDS,
    ]
  : IS_OBITOBUFF
    ? ALL_SLASH_COMMANDS.filter(
        (cmd) => !OBITOBUFF_REMOVED_COMMAND_IDS.has(cmd.id),
      )
    : ALL_SLASH_COMMANDS.filter(
        (cmd) => !OBITOBUFF_ONLY_COMMAND_IDS.has(cmd.id),
      )
