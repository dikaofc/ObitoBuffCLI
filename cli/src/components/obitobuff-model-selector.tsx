...
import { ObitobuffReferralBanner } from './obitobuff-referral-banner'
import {
  OBITOBUFF_GLM_V52_MODEL_ID,
  OBITOBUFF_PREMIUM_SESSION_LIMIT,
...
import { startObitobuffSession } from '../hooks/use-obitobuff-session'
import { useNow } from '../hooks/use-now'
import { useObitobuffModelStore } from '../state/obitobuff-model-store'
...
} from '../utils/obitobuff-model-navigation'
import { formatSessionUnits } from '../utils/format-session-units'
import {
  formatObitobuffPremiumResetCountdown,
  getObitobuffPremiumResetAt,
} from '../utils/obitobuff-premium-reset'
import { isPlainEnterKey } from '../utils/terminal-enter-detection'

import type {
  ObitobuffAccessTier,
  ObitobuffModelOption,
} from '@codebuff/common/constants/obitobuff-models'
import type { ObitobuffReferralFocusTarget } from './obitobuff-referral-banner'
import type {
  BoxRenderable,
  KeyEvent,
  ScrollBoxRenderable,
...
type Section = {
  ...
  label: string
  models: readonly ObitobuffModelOption[]
}
...
const TOGGLE_ID = '__obitobuff_toggle__'
 ...
 */
interface ObitobuffModelSelectorProps {
 ...
 *  model, so it reaches the user through ObitobuffReferralBanner instead. */
function gridModels(
  accessTier: ObitobuffAccessTier,
): readonly ObitobuffModelOption[] {
  return getObitobuffModelsForAccessTier(accessTier).filter(
    (m) => !isObitobuffGlmV52ModelId(m.id),
  )
}
...
export function obitobuffCliOfferedModelIds(
  accessTier: ObitobuffAccessTier,
): readonly string[] {
  return [...gridModels(accessTier).map((m) => m.id), OBITOBUFF_GLM_V52_MODEL_ID]
}

export const ObitobuffModelSelector: React.FC<ObitobuffModelSelectorProps> = ({
  maxHeight,
  onExpandedChange,
  belowToggle,
}) => {
  ...
  const selectedModel = useObitobuffModelStore((s) => s.selectedModel)
  const setSelectedModel = useObitobuffModelStore((s) => s.setSelectedModel)
  ...
  const deploymentAvailabilityLabel = useMemo(
    () => getObitobuffDeploymentAvailabilityLabel(new Date(now)),
    [now],
  )
  ...
  const offers = useMemo(
    () =>
      getLimitedModelOffers(session).filter((offer) =>
        isSupportedObitobuffModelId(offer.model),
      ),
    [session],
  )
  const offerModels = useMemo(
    () => offers.map((offer) => getObitobuffModel(offer.model)),
    [offers],
  )
  ...
  const premiumLimit = sharedRateLimit?.limit ?? OBITOBUFF_PREMIUM_SESSION_LIMIT
  ...
  // countdown is meaningful even at zero used — getObitobuffPremiumResetAt falls
  // back to the next day boundary when the server hasn't sent a resetAt yet.
  const premiumResetCountdown = formatObitobuffPremiumResetCountdown(
    getObitobuffPremiumResetAt({ rateLimitsByModel, nowMs: now }),
    now,
  )

  const recommendedModel = useMemo(() => {
    const id = getRecommendedObitobuffModelId(accessTier, { premiumExhausted })
  ...
  // one Enter away), which is what getObitobuffModelSupersededBy guarantees by
  // resolving against the models actually on screen.
  const supersededNoticeFor = useCallback(
    (model: ObitobuffModelOption): string | undefined =>
      ...
      [
        {
          key: 'premium',
          label: 'PREMIUM',
          models: availableModels.filter((m) => isObitobuffPremiumModelId(m.id)),
        },
        {
          key: 'unlimited',
          label: 'UNLIMITED',
          models: availableModels.filter(
            (m) => !isObitobuffPremiumModelId(m.id),
          ),
        },
      ] satisfies readonly Section[]
  ...
  // models" reaches its buttons; nextObitobuffModelId wraps back to the top).
  ...
  useEffect(() => {
    ...
    const selectionIsValid =
      ...
      (isObitobuffGlmV52ModelId(selectedModel) &&
        (referral?.weeklySessionsRemaining ?? 0) > 0)
    if (
      isLanding &&
      (!selectionIsValid ||
        !isObitobuffModelAvailable(selectedModel, new Date(now)))
    ) {
  ...
  const { compactNames, buttonOuterWidth, buttonInnerWidth, nameColumnWidth } =
    useMemo(() => {
      ...
      const detailsParts = (model: ObitobuffModelOption): number[] => {
        const parts: number[] = []
      ...
      const noticeLineLen = (m: ObitobuffModelOption) =>
      ...
      const compactLabelLen = (m: ObitobuffModelOption) =>
        2 +
      ...
      // stretches the card.
      const detailsLineLen = (m: ObitobuffModelOption) =>
      ...
      const innerWidth = (labelLen: (m: ObitobuffModelOption) => number) =>
        Math.max(
          ...widthModels.map((m) =>
  ...
  const estimatedModelHeight = useMemo(() => {
    let y = 0
    const rowHeight = (m: ObitobuffModelOption) =>
  ...
  const isJoinable = useCallback(
    (modelId: string) => {
      if (!isObitobuffModelAvailable(modelId, new Date(now))) return false
  ...
  const pick = useCallback(
    (modelId: string) => {
      ...
      setPending(modelId)
      startObitobuffSession(modelId).finally(() => setPending(null))
    },
  ...
  useKeyboard(
    useCallback(
      (key: KeyEvent) => {
        if (pending) return
        const name = key.name ?? ''
        const direction = obitobuffModelNavigationDirectionForKey(key)
        ...
        if (!direction) return
        const targetId = nextObitobuffModelId({
          modelIds: navIds,
          focusedId,
          direction,
        })
  ...
  const renderModelButton = (
    ...
    // while closed (the label flips inside getObitobuffDeploymentAvailabilityLabel),
    ...
    // completions layer sends) — see ObitobuffModelOption.reasoningEffort.
  ...
  const offerUserResetCountdown =
    offerUserResetAt && Number.isFinite(offerUserResetAt.getTime())
      ? formatObitobuffPremiumResetCountdown(offerUserResetAt, now)
      : null
