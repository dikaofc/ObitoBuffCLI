...
import { REFERRAL_CLI_DAILY_SESSION_BONUS_CAP } from '@codebuff/common/constants/obitobuff-referral-tiers'
 ...
 *  "X invited you to try Obitobuff!". */
...
const COPY_FOCUS_ID = '__obitobuff_referral_copy__'
const GLM_FOCUS_ID = '__obitobuff_referral_glm__'
...
const DASHBOARD_FOCUS_ID = '__obitobuff_referral_dashboard__'
const BUTTON_HORIZONTAL_CHROME = 6 // two border + four padding columns

export interface ObitobuffReferralFocusTarget {
  id: string
  activate: () => void
}
...
const shouldStackObitobuffReferralActions = (width: number): boolean =>
  width < 70
 ...
 */
interface ObitobuffReferralBannerProps {
  width: number
  referral: ObitobuffReferralInfo
  /** A live GLM promo, or undefined. Undefined is the ordinary state. */
  glmPromo?: ObitobuffGlmPromo
  accessTier: ObitobuffAccessTier
  focusedId: string
  onFocusTargetsChange: (targets: ObitobuffReferralFocusTarget[]) => void
}

export const ObitobuffReferralBanner: React.FC<ObitobuffReferralBannerProps> = ({
  width,
  referral,
  glmPromo,
  accessTier,
  ...
  const useGlm = useCallback(() => {
    ...
    setJoining(true)
    startObitobuffSession(OBITOBUFF_GLM_V52_MODEL_ID).finally(() => {
  ...
  const resetsIn = formatObitobuffPremiumResetCountdown(
  ...
  const stackActions = shouldStackObitobuffReferralActions(width)
