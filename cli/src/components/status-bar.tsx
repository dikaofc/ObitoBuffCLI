...
} from '../utils/obitobuff-session-display'
...
export const StatusBar = ({
  ...
  const sessionProgress = useObitobuffSessionProgress(obitobuffSession)
  const isUnlimited =
    obitobuffSession?.status === 'active' && !obitobuffSession.rateLimit
    ...
    switch (statusIndicatorState.kind) {
      ...
      case 'idle':
        if (sessionProgress !== null) {
          ...
          const modelName =
            obitobuffSession?.status === 'active'
              ? getObitobuffModel(obitobuffSession.model).displayName
              : null
          return (
            <span
              fg={
            ...
            >
              ...
              {isUnlimited
                ? 'unlimited'
                : formatObitobuffSessionRemaining(sessionProgress.remainingMs)}
            </span>
          )
        }
        return null
  ...
  // obitobuff session fill is visible (otherwise the fill would float over
  // transparent space).
  const hasContent =
          ...
          !isUnlimited && (
            <text style={{ wrapMode: 'none' }}>
              <span fg={theme.warning} attributes={TextAttributes.BOLD}>
                {formatObitobuffSessionCountdown(sessionProgress.remainingMs)}
              </span>
            </text>
          )}
      </box>
