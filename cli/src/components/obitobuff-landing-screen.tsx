...
import {
  refreshObitobuffLandingMetadata,
  takeOverObitobuffSession,
} from '../hooks/use-obitobuff-session'
import { useObitobuffCtrlCExit } from '../hooks/use-obitobuff-ctrl-c-exit'
import { useObitobuffStreakQuery } from '../hooks/use-obitobuff-streak-query'
...
import {
  ...
  OBITOBUFF_STREAK_LABEL_GAP,
  fitsObitobuffStreakOnHeadingRow,
  getObitobuffStreakBonusNoteForLayout,
  getObitobuffStreakLine,
} from '../utils/obitobuff-streak-line'
import { formatSessionUnits } from '../utils/format-session-units'
import { isPlainEnterKey } from '../utils/terminal-enter-detection'
import { getLogoAccentColor, getLogoBlockColor } from '../utils/theme-system'
import { INVERTED_CTA_FG } from '../utils/ui-constants'
import {
  OBITOBUFF_ENABLE_STREAK_IN_UI,
  OBITOBUFF_LIMITED_SESSION_LIMIT,
...
import { getObitobuffModelAvailabilityNotice } from '@codebuff/common/util/obitobuff-model-availability'
import { formatObitobuffHardBlockedPrivacySignals } from '@codebuff/common/util/obitobuff-privacy'

import type { ObitobuffStreakLine } from '../utils/obitobuff-streak-line'
import type { ObitobuffSessionFailure } from '../state/obitobuff-session-store'
import type { ObitobuffSessionResponse } from '../types/obitobuff-session'
import type { KeyEvent } from '@opentui/core'

interface ObitobuffLandingScreenProps {
...
// gets asked. The copy itself is shared with Obitobuff Desktop's model menu; see
// `getObitobuffModelAvailabilityNotice` for the tone rules it follows.
...
function getTakeoverErrorMessage(failure: ObitobuffSessionFailure): string {
  if (failure.type === 'http' && failure.statusCode === 503) {
    return "Obitobuff is busy and couldn't complete the takeover yet."
  }
  if (failure.type === 'timeout') {
    return failure.outcomeUnknown
      ...
      : failure.retry
        ? 'The takeover request timed out while Obitobuff was busy.'
        : 'The takeover request timed out.'
  }
  if (failure.outcomeUnknown) {
    return "Obitobuff couldn't confirm whether the takeover succeeded. Check the warning, then retry if you still want to take over."
  }
...
}> = ({ failure, onTakeOver = takeOverObitobuffSession }) => {
  ...
  return (
    ...
    >
      <text style={{ fg: theme.foreground }} attributes={TextAttributes.BOLD}>
        Obitobuff is already running
      </text>

      <text style={{ fg: theme.muted }}>
        Only one obitobuff instance is allowed at a time.
      </text>
...
const streakSpans = (
  ...
  <span key="dots" fg={theme.primary}>
    {`${' '.repeat(OBITOBUFF_STREAK_LABEL_GAP)}${line.dots}`}
  </span>,
]
 ...
 *  one (only when it fits — see fitsObitobuffStreakOnHeadingRow).
 *
...
export const LandingHeadingRow: React.FC<{
  streakLine: ObitobuffStreakLine | null
  marginBottom: number
  ...
  return (
    <text style={{ marginBottom, wrapMode: 'word' }}>
      ...
      {streakLine && [
        <span key="gap">{' '.repeat(OBITOBUFF_STREAK_INLINE_GAP)}</span>,
...
export const ObitobuffLandingScreen: React.FC<ObitobuffLandingScreenProps> = ({
  session,
  failure,
}) => {
  const theme = useTheme()
  ...
  // startObitobuffSession, which POSTs and transitions straight to 'active' (chat).
  const isLanding = session?.status === 'none'
  const streakQuery = useObitobuffStreakQuery({
    enabled: OBITOBUFF_ENABLE_STREAK_IN_UI && isLanding,
  })
  ...
  const showStreakIndicator = OBITOBUFF_ENABLE_STREAK_IN_UI && isLanding
  ...
  const streakLine = showStreakIndicator ? getObitobuffStreakLine(streak) : null
  ...
  const sessionResetAt = getObitobuffPremiumResetAt({
    rateLimitsByModel,
    nowMs: now,
  })
  const sessionResetAtMs = sessionResetAt.getTime()
  const sessionResetCountdown = formatObitobuffPremiumResetCountdown(
    sessionResetAt,
    now,
  )
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      refreshObitobuffLandingMetadata().catch(() => {})
    }, delayMs)
        ...
        >
            ...
            <>
              ...
              <text style={{ fg: theme.muted, wrapMode: 'word' }}>
                {session.countryBlockReason === 'anonymous_network' ? (
                  <>
                    We detected{' '}
                    {formatObitobuffHardBlockedPrivacySignals(
                    ...
                    . Obitobuff can't be used from VPN, proxy, or Tor traffic.
                    Disable it and restart Obitobuff to try again.
                  </>
                ) : session.countryCode === 'UNKNOWN' ? (
                  <>
                    We couldn't verify an eligible location for this request.
                    VPN, Tor, proxy, or unknown-location traffic can't use
                    obitobuff. Press Ctrl+C to exit.
                  </>
                ) : (
                  <>
                    We detected your location as{' '}
                    <span fg={theme.foreground}>{session.countryCode}</span>,
                    which is outside the countries where obitobuff is currently
          ...
          {session?.status === 'banned' && (
            <>
              ...
              <text style={{ fg: theme.muted, wrapMode: 'word' }}>
                This account has been suspended and can't use obitobuff. If you
          ...
          {session?.status === 'ip_capped' && (
            <>
              <text style={{ fg: theme.secondary, marginBottom: 1 }}>
                🚦 Too many Obitobuff sessions on this network
              </text>
              <text style={{ fg: theme.muted, wrapMode: 'word' }}>
                {session.activeUsersForIp} other people are already using
                Obitobuff from your network, which is the most we allow at once.
                Try again in{' '}
                <span fg={theme.foreground}>
                  {formatRetryAfter(session.retryAfterMs)}
                </span>
