import {
  ...
  type ObitobuffModelOption,
} from '@codebuff/common/constants/obitobuff-models'
import { getRateLimitsByModel } from '@codebuff/common/types/obitobuff-session'
...
/**
 * Replaces the chat input when the obitobuff session has ended. Captures
...
export const SessionEndedBanner: React.FC<SessionEndedBannerProps> = ({
  ...
  // count.
  const premiumQuota = useObitobuffSessionStore(
  ...
  const isQuotaExhausted = premiumQuota
    ...
    : false
  const accessTier = useObitobuffSessionStore((s) =>
  ...
  const continueOnFallback =
    isQuotaExhausted &&
    accessTier !== 'limited' &&
    isObitobuffPremiumModelId(selectedModel)
  const fallbackModel: ObitobuffModelOption | undefined =
    SUPPORTED_OBITOBUFF_MODELS.find((m) => m.id === FALLBACK_OBITOBUFF_MODEL_ID)
  ...
  const pickNewModel = useCallback(() => {
    ...
    // new session. app.tsx swaps us into <ObitobuffLandingScreen> on the
  ...
  const startSameChatSession = useCallback(() => {
    ...
    refreshObitobuffSession().catch(() => setPendingAction(null))
