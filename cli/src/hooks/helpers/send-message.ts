import { OBITOBUFF_PROVIDER_USAGE_MESSAGE } from '@codebuff/common/constants/obitobuff-errors'
import { getErrorObject } from '@codebuff/common/util/error'

import {
  markObitobuffSessionCountryBlocked,
  markObitobuffSessionEnded,
  markObitobuffSessionSuperseded,
  refreshObitobuffSession,
} from '../use-obitobuff-session'
...
import {
  ...
  getObitobuffGateErrorKind,
  getObitobuffRateLimitErrorMessage,
  isOutOfCreditsError,
  isObitobuffProviderUsageError,
...
export const handleRunCompletion = (params: {
  ...
  /** Whether the run streamed any content before finishing. A obitobuff gate
  ...
  if (output.type === 'error') {
    if (IS_OBITOBUFF && isObitobuffProviderUsageError(output)) {
      updater.setError(OBITOBUFF_PROVIDER_USAGE_MESSAGE)
      finalizeAfterError()
      return
    }
    ...
    const gateKind = getObitobuffGateErrorKind(output)
    if (gateKind) {
      handleObitobuffGateError(gateKind, updater, {
    ...
    const obitobuffRateLimitMessage = IS_OBITOBUFF
      ? getObitobuffRateLimitErrorMessage(output)
      : null
    if (obitobuffRateLimitMessage) {
      updater.setError(obitobuffRateLimitMessage)
      finalizeAfterError()
      return
    }
...
export const handleRunError = (params: {
  ...
  if (IS_OBITOBUFF && isObitobuffProviderUsageError(error)) {
    updater.setError(OBITOBUFF_PROVIDER_USAGE_MESSAGE)
    return
  }
  ...
  const gateKind = getObitobuffGateErrorKind(error)
  if (gateKind) {
    handleObitobuffGateError(gateKind, updater, {
  ...
  const obitobuffRateLimitMessage = IS_OBITOBUFF
    ? getObitobuffRateLimitErrorMessage(error)
    : null
  if (obitobuffRateLimitMessage) {
    updater.setError(obitobuffRateLimitMessage)
    return
 ...
 */
function handleObitobuffGateError(
  kind: ReturnType<typeof getObitobuffGateErrorKind>,
  updater: BatchedMessageUpdater,
  ...
  switch (kind) {
    ...
    case 'waiting_room_queued':
      ...
      // "let's start fresh".
      refreshObitobuffSession().catch(() => {})
      return
    case 'session_superseded':
      updater.setError(
        'Another obitobuff CLI took over this account. Close the other instance, then restart.',
      )
