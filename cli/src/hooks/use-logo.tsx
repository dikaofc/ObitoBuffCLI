...
export const useLogo = ({
  ...
  const rawLogoString = useMemo(() => {
    if (maxHeight != null && maxHeight < ASCII_LOGO_LINES) {
      return IS_OBITOBUFF ? 'OBITOBUFF' : 'CODEBUFF'
    }
  ...
  const textBlock = useMemo(() => {
    if (rawLogoString === 'CODEBUFF' || rawLogoString === 'OBITOBUFF') {
  ...
  const component = useMemo(() => {
    ...
    if (rawLogoString === 'CODEBUFF' || rawLogoString === 'OBITOBUFF') {
      ...
      // the terminal is narrow), keep it to the bare brand name — "Obitobuff
