...
export const useChatInput = ({
  ...
  // In Obitobuff, the toggle is always hidden, so never reserve width for it.
  // In non-Obitobuff: hide in bash mode, compact height, or narrow width.
  const estimatedToggleWidth =
    IS_OBITOBUFF || inputMode !== 'default' || isCompactHeight || isNarrowWidth
      ? 0
