...
export function useAuthQuery(deps: UseAuthQueryDeps = {}) {
  ...
  return useQuery({
    ...
    // Local mode runs entirely on configured providers: never validate against
    // the backend (a stale local credential would 401 and flip the UI to the
    // login modal even though no login is required).
    enabled: !IS_LOCAL_MODE && !!apiKey,
