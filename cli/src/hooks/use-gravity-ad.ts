...
// legacy wire name for the obitobuff landing screen; 'cli_chat' is the inline
...
type GravityAdOptionsBase = {
  enabled?: boolean
  /** Skip the "wait for first user message" gate. Used by the obitobuff
...
export const useGravityAd = (options?: GravityAdOptions): GravityAdState => {
  ...
  // Obitobuff always shows ads even on compact screens (ads are mandatory there).
  const isFreeMode = IS_OBITOBUFF

  // Skip ads on very compact screens unless we're in Obitobuff (where ads are mandatory)
  ...
  const recordImpressionOnce = (ad: AdResponse): void => {
    ...
    const recordLocalImpression = async (): Promise<void> => {
      ...
      // Include mode in request - Obitobuff should not grant credits (no balance concept).
...
function getCliAdRequestUserAgent(): string {
  const product = IS_OBITOBUFF ? 'Obitobuff-CLI' : 'Codebuff-CLI'
