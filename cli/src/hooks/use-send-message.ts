...
import {
  getObitobuffInstanceId,
  markObitobuffSessionEnded,
} from './use-obitobuff-session'
import { getCodebuffClient } from '../utils/codebuff-client'
import { AGENT_MODE_TO_COST_MODE, IS_OBITOBUFF } from '../utils/constants'
import { createEventHandlerState } from '../utils/create-event-handler-state'
import { createRunConfig } from '../utils/create-run-config'
import { getAgentIdForMode } from '../utils/obitobuff-agent-selection'
...
interface UseSendMessageOptions {
  ...
  resumeQueue?: () => void
  /** Put a message back at the head of the queue. Used by the obitobuff
...
export const useSendMessage = ({
  ...
  const sendMessage = useCallback<SendMessageFn>(
    async ({ content, agentMode, postUserMessage, attachments }) => {
      ...
      // Obitobuff run-start guard: without a live session slot the server
      ...
      if (IS_OBITOBUFF && !getObitobuffInstanceId()) {
        markObitobuffSessionEnded()
      ...
      if (!client) {
        ...
        const brandName = IS_OBITOBUFF ? 'Obitobuff' : 'Codebuff'
        setMessages((prev) => [
          ...prev,
          createErrorChatMessage(
      ...
      try {
        ...
        const obitobuffInstanceId = getObitobuffInstanceId()
        const runConfig = createRunConfig({
          ...
          extraCodebuffMetadata:
            IS_OBITOBUFF && obitobuffInstanceId
              ? { obitobuff_instance_id: obitobuffInstanceId }
              : undefined,
