...
import { holdsLiveObitobuffSlot } from '../../utils/obitobuff-session-api'
...
const activeSession: ObitobuffSessionResponse = {
  status: 'active',
  accessTier: 'full',
  instanceId: 'inst-1',
...
describe('holdsLiveObitobuffSlot', () => {
  test('active session holds a slot', () => {
    expect(holdsLiveObitobuffSlot(activeSession)).toBe(true)
  })

  test('ended WITH instance id (grace window) still holds a slot', () => {
    expect(
      holdsLiveObitobuffSlot({ status: 'ended', instanceId: 'inst-1' }),
    ).toBe(true)
  })

  test('ended WITHOUT instance id (post-grace) does not hold a slot', () => {
    expect(holdsLiveObitobuffSlot({ status: 'ended' })).toBe(false)
  })

  test('null session does not hold a slot', () => {
    expect(holdsLiveObitobuffSlot(null)).toBe(false)
  })

  test('pre-join and terminal states do not hold a slot', () => {
    expect(holdsLiveObitobuffSlot({ status: 'none' })).toBe(false)
    expect(holdsLiveObitobuffSlot({ status: 'superseded' })).toBe(false)
    expect(
      holdsLiveObitobuffSlot({ status: 'takeover_prompt', model: 'm' }),
    ).toBe(false)
  })
})
