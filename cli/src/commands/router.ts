...
export async function routeUserPrompt(
  ...
  // so combined DAU is a single unique-users query. Obitobuff-only: codebuff
  // CLI usage is intentionally excluded.
  if (IS_OBITOBUFF) {
    const obitobuffSession = useObitobuffSessionStore.getState().session
    trackEvent(AnalyticsEvent.MESSAGE_SENT, {
      surface: 'cli',
      accessTier:
        obitobuffSession && 'accessTier' in obitobuffSession
          ? obitobuffSession.accessTier
          : 'unknown',
      mode: agentMode,
      inputMode,
      inputLength: trimmed.length,
