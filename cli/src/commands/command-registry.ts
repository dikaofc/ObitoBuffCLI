...
import { returnToObitobuffLanding } from '../hooks/use-obitobuff-session'
...
const OBITOBUFF_REMOVED_COMMANDS = new Set([
  'ads:enable',
  'ads:disable',
  'usage',
  'subscribe',
...
const ALL_COMMANDS: CommandDefinition[] = [
  ...
  // Mode commands generated from AGENT_MODES (excluded in Obitobuff)
  ...(IS_OBITOBUFF ? [] : AGENT_MODES).map((mode) =>
    defineCommandWithArgs({
  ...
  // /end-session (obitobuff-only) — end the active session early and drop back
  // to the model picker. The hook flips status to 'none', which unmounts
  // <Chat> and mounts <ObitobuffLandingScreen>, where the user picks a model
  ...
  defineCommand({
    ...
    handler: (params) => {
      ...
      params.saveToHistory(params.inputValue.trim())
      clearInput(params)
      returnToObitobuffLanding({ resetChat: true }).catch(() => {
...
/**
 * /model — local-mode model switcher. Lists the models from
 * obitobuff.config.json, or switches to one with `/model <id>`.
 */
function createModelSwitchCommand(): CommandDefinition {
    ...
    handler: (params, args) => {
      ...
      if (options.length === 0) {
        params.setMessages((prev) => [
          ...prev,
          getSystemMessage(
            'No models configured. Add a providers section to obitobuff.config.json.',
          ),
        ])
        return
      }
      ...
      if (trimmed) {
        const match = options.find(
          (option) =>
            option.id === trimmed ||
            option.name === trimmed ||
            option.provider === trimmed,
        )
        if (match) {
          useObitobuffModelStore.getState().setSelectedModel(match.id)
          saveObitobuffModelPreference(match.id)
          params.setMessages((prev) => [
            ...prev,
            getUserMessage(params.inputValue.trim()),
            getSystemMessage(
              `Switched to ${match.name} (via ${match.provider}).`,
            ),
          ])
          return
        }
        params.setMessages((prev) => [
          ...prev,
          getSystemMessage(
            `Unknown model "${trimmed}". Available: ${options
      ...
      const current = getSelectedObitobuffModel()
      const list = options
        .map((option) => {
          const label = option.name !== option.id ? ` (${option.name})` : ''
          const marker = option.id === current ? '▸ ' : '  '
          return `${marker}${option.id}${label} — ${option.provider}`
        })
        .join('\n')
      params.setMessages((prev) => [
        ...prev,
        getSystemMessage(
          `Current model: ${current}\n\nAvailable models:\n${list}\n\nSwitch with /model <id> — no account or session needed.`,
        ),
      ])
    },
  })
...
let commands: CommandDefinition[] = IS_OBITOBUFF
  ? ALL_COMMANDS.filter((cmd) => !OBITOBUFF_REMOVED_COMMANDS.has(cmd.name))
  : ALL_COMMANDS.filter((cmd) => !OBITOBUFF_ONLY_COMMANDS.has(cmd.name))

if (IS_LOCAL_MODE) {
  // Local mode has no server session to end, and /model switches between
  // configured providers instead.
  commands = commands.filter((cmd) => cmd.name !== 'end-session')
  commands.push(createModelSwitchCommand())
}

export const COMMAND_REGISTRY: CommandDefinition[] = commands
