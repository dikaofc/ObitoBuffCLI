...
async function main(): Promise<void> {
  ...
  // --model overrides the startup model selection (local config models in
  // local mode, free catalog models otherwise).
  if (model) {
    useObitobuffModelStore.getState().setSelectedModel(model)
  }
  ...
  if (IS_OBITOBUFF && process.platform === 'win32') {
  ...
  const AppWithAsyncAuth = () => {
    ...
    React.useEffect(() => {
      // Local mode runs entirely on configured providers: no account needed.
      if (IS_LOCAL_MODE) {
