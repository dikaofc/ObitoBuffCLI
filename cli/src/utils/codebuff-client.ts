...
export async function getCodebuffClient(): Promise<CodebuffClient | null> {
  if (!clientInstance) {
    ...
    // Local mode talks only to the providers configured in obitobuff.config.json,
    // so no account token is needed. A placeholder keeps the client constructible;
    // LLM requests never see it (they route to the configured provider).
    const effectiveApiKey = apiKey ?? (IS_LOCAL_MODE ? 'local-mode' : undefined)
