...
const createCodeSearcherBlock = (
  ...
  params: {
    searchQueries: [
      { pattern: 'OBITOBUFF_MODEL_SELECTOR_MODELS' },
      { pattern: 'OBITOBUFF_MODEL_SELECTOR_MODEL_IDS' },
      { pattern: 'DEFAULT_OBITOBUFF_MODEL_ID' },
    ],
  },
  blocks: [],
  ...options,
...
describe('code search summary helpers', () => {
  ...
  test('handles singular labels', () => {
    const agentBlock = createCodeSearcherBlock({
      params: {
        searchQueries: [{ pattern: 'DEFAULT_OBITOBUFF_MODEL_ID' }],
      },
