...
function protocolFilesForThisProcess(): Set<string> {
  const prefix = `obitobuff-terminal-command-broker-${process.pid}-`
  return new Set(
...
describe('terminal command broker', () => {
  ...
  test('requires its private environment marker and flag before --', () => {
    expect(
      isTerminalCommandBrokerInvocation(
        ['obitobuff', '--terminal-command-broker'],
    ...
    expect(
      isTerminalCommandBrokerInvocation(['obitobuff'], {
        CODEBUFF_TERMINAL_COMMAND_BROKER: '1',
      }),
    ).toBe(false)
  })
  ...
  test('accepts protocol files only at the constrained temp path', () => {
    const validPath = path.join(
      tmpdir(),
      `obitobuff-terminal-command-broker-${process.pid}-${crypto.randomUUID()}.json`,
    )
    expect(
      protocolPathFromEnv({
    ...
    const broker = createTerminalCommandBroker({
      invocation: () => ({
        executable: path.join(
          tmpdir(),
          `missing-obitobuff-broker-${crypto.randomUUID()}`,
        ),
        args: [],
      }),
      reportFailure: (failure) => failures.push(failure),
  ...
  test('adds recovery guidance when spawning the helper throws', async () => {
    ...
    ).rejects.toThrow('Restart Obitobuff and try again.')
  })
  ...
  test('adds recovery guidance when the helper exits before responding', async () => {
    ...
    const broker = createTerminalCommandBroker({
      invocation: () => ({
        ...
        args: [
          path.join(
            tmpdir(),
            `missing-obitobuff-entry-${crypto.randomUUID()}.ts`,
          ),
        ],
      }),
      reportFailure: (failure) => failures.push(failure),
    ...
    expect(failureMessage).toContain('Restart Obitobuff and try again.')
    expect(failures).toEqual([
  ...
  test('does not add broker recovery guidance to a command spawn failure', async () => {
    const missingCwd = path.join(
      tmpdir(),
      `missing-obitobuff-cwd-${crypto.randomUUID()}`,
    )
    ...
    expect(failureMessage).not.toContain('Restart Obitobuff and try again.')
  })
