...
import {
  ...
  getObitobuffStreakInlineWidth,
  getObitobuffStreakLine,
} from '../obitobuff-streak-line'
...
// glyphs get.
describe('getObitobuffStreakLine', () => {
  test('hides the row for new / lapsed users (streak <= 0)', () => {
    expect(getObitobuffStreakLine(0)).toBeNull()
    expect(getObitobuffStreakLine(-1)).toBeNull()
  })

  test('labels and fills dots for an active streak', () => {
    expect(getObitobuffStreakLine(2)).toEqual({
      label: '2 day streak',
      dots: '••·····',
  ...
  test('"day" stays singular as a compound modifier', () => {
    expect(getObitobuffStreakLine(1)?.label).toBe('1 day streak')
    expect(getObitobuffStreakLine(5)?.label).toBe('5 day streak')
  })

  test('fills the whole week on a 7-day milestone', () => {
    expect(getObitobuffStreakLine(7)).toEqual({
      label: '7 day streak',
      dots: '•••••••',
  ...
  test('stays full and gains a "+" once the streak passes the week', () => {
    expect(getObitobuffStreakLine(9)).toEqual({
      label: '9 day streak',
      dots: '•••••••+',
      progress: { filled: 7, total: 7, beyond: true },
    })
    expect(getObitobuffStreakLine(19)).toEqual({
      label: '19 day streak',
      dots: '•••••••+',
      progress: { filled: 7, total: 7, beyond: true },
    })
...
describe('fitsObitobuffStreakOnHeadingRow', () => {
  const headingWidth = 'Start coding for free'.length
  const line = getObitobuffStreakLine(18)!
  // "18 day streak" + 2 + "•••••••+"
  const inlineWidth = getObitobuffStreakInlineWidth(line)
  const exact = headingWidth + OBITOBUFF_STREAK_INLINE_GAP + inlineWidth
  ...
  test('measures an empty slot as the day-one streak it will become', () => {
    const dayOne = getObitobuffStreakLine(1)!
    const width = headingWidth + OBITOBUFF_STREAK_INLINE_GAP
    expect(
      fitsObitobuffStreakOnHeadingRow({
        line: null,
        headingWidth,
        availableWidth: width + getObitobuffStreakInlineWidth(dayOne),
      }),
    ).toBe(true)
    expect(
      fitsObitobuffStreakOnHeadingRow({
        line: null,
        headingWidth,
        availableWidth: width + getObitobuffStreakInlineWidth(dayOne) - 1,
      }),
    ).toBe(false)
  })
  ...
  test('accounts for the label growing with the day count', () => {
    const long = getObitobuffStreakLine(365)!
    expect(getObitobuffStreakInlineWidth(long)).toBeGreaterThan(inlineWidth)
    expect(
      fitsObitobuffStreakOnHeadingRow({
        line: long,
        headingWidth,
...
describe('getObitobuffStreakBonusNote', () => {
  ...
  test('the GLM streak count grows per completed 7 days, capped at 4', () => {
    expect(getObitobuffStreakBonusNote({ streak: 14, accessTier: 'full' })).toBe(
      '🎁 Streak perk: +1 bonus session every day + 2 GLM 5.2 sessions each day',
    )
    expect(getObitobuffStreakBonusNote({ streak: 35, accessTier: 'full' })).toBe(
  ...
  test('limited access advertises only the daily session perk', () => {
    const note = getObitobuffStreakBonusNote({
      streak: 14,
      accessTier: 'limited',
    })
...
describe('getObitobuffStreakBonusNoteForLayout', () => {
  const params = {
    streak: 7,
