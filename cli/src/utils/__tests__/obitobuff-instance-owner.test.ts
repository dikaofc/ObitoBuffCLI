...
} = await import('../obitobuff-instance-owner')

describe('obitobuff instance owner', () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), 'obitobuff-owner-'))
  ...
  test('does not classify the current process as dead', () => {
    recordObitobuffInstanceOwner('inst-current')

    expect(
      isObitobuffInstanceOwnedByDeadLocalProcess('inst-current'),
    ).toBe(false)
  })
  ...
  test('classifies a matching owner with a dead pid as dead', () => {
    ...
    expect(isObitobuffInstanceOwnedByDeadLocalProcess('inst-dead')).toBe(true)
  })
