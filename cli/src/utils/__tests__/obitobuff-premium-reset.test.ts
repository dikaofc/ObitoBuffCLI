...
} from '../obitobuff-premium-reset'

describe('obitobuff premium reset helpers', () => {
  test('uses server resetAt when it is in the future', () => {
    const nowMs = Date.parse('2026-05-11T20:00:00.000Z')
    const resetAt = getObitobuffPremiumResetAt({
      nowMs,
      rateLimitsByModel: {
        'test/model': {
          model: 'test/model',
  ...
  test('keeps expired server resetAt instead of rolling stale quota forward', () => {
    ...
    expect(resetAt.toISOString()).toBe('2026-05-12T07:00:00.000Z')
    expect(formatObitobuffPremiumResetCountdown(resetAt, nowMs)).toBe('now')
  })
  ...
  test('formats hours and minutes left', () => {
    ...
    expect(formatObitobuffPremiumResetCountdown(resetAt, nowMs)).toBe('11h 30m')
  })
  ...
  test('formats sub-hour reset countdowns', () => {
    ...
    expect(formatObitobuffPremiumResetCountdown(resetAt, nowMs)).toBe('30m')
  })
})
