...
test('full-tier referral GLM reaches the session POST header unchanged', async () => {
  ...
  const resolved = resolveObitobuffModelForAccessTier(
    OBITOBUFF_GLM_V52_MODEL_ID,
    'full',
  )

  await callObitobuffSession('POST', 'test-token', { model: resolved })

  expect(fetchSpy).toHaveBeenCalledTimes(1)
  const [, init] = fetchSpy.mock.calls[0]!
  expect(new Headers(init?.headers).get('x-obitobuff-model')).toBe(
    OBITOBUFF_GLM_V52_MODEL_ID,
  )
})
...
test('compact GET sends the compact-session header', async () => {
  ...
  await callObitobuffSession('GET', 'test-token', {
    instanceId: 'i1',
    compact: true,
  })

  const [, init] = fetchSpy.mock.calls[0]!
  expect(new Headers(init?.headers).get('x-obitobuff-compact-session')).toBe('1')
})
...
test('does not repeat a takeover POST after an ambiguous timeout', () => {
  ...
  expect(classifyObitobuffSessionRequestFailure('POST', timeout)).toBe('unknown')
  expect(classifyObitobuffSessionRequestFailure('GET', timeout)).toBe('retry')
})
...
test('retries POST responses that cannot represent a committed takeover', async () => {
    ...
    Response.json(
      {
        ...
        message: 'Obitobuff session service is busy. Please retry shortly.',
      },
      { status: 503, headers: { 'retry-after': '10' } },
    ),
  )

  await expect(callObitobuffSession('POST', 'test-token')).rejects.toMatchObject({
    statusCode: 503,
    retryAfterMs: 10_000,
  ...
  expect(
    classifyObitobuffSessionRequestFailure(
      'POST',
      new ObitobuffSessionRequestError(
        'busy',
        503,
        10_000,
        'service_overloaded',
  ...
  expect(
    classifyObitobuffSessionRequestFailure(
      'POST',
      new ObitobuffSessionRequestError('generic proxy 503', 503, 10_000),
    ),
  ).toBe('retry')
  expect(
    classifyObitobuffSessionRequestFailure(
      'POST',
      new ObitobuffSessionRequestError('request timeout', 408, 10_000),
    ),
  ).toBe('retry')
  expect(
    classifyObitobuffSessionRequestFailure(
      'POST',
      new ObitobuffSessionRequestError('edge rate limit', 429, 10_000),
    ),
  ).toBe('retry')
})
...
test('stops on terminal 4xx responses', () => {
  expect(
    classifyObitobuffSessionRequestFailure(
      'POST',
      new ObitobuffSessionRequestError('unauthorized', 401),
    ),
  ).toBe('stop')
  expect(
    classifyObitobuffSessionRequestFailure(
      'GET',
      new ObitobuffSessionRequestError('not found', 404),
    ),
  ).toBe('stop')
})
...
test('marks response loss and server errors after a POST as unknown outcomes', () => {
  ...
  expect(
    classifyObitobuffSessionRequestFailure(
      'POST',
      new ObitobuffSessionRequestError('internal error', 500),
    ),
  ).toBe('unknown')
  expect(
    classifyObitobuffSessionRequestFailure(
