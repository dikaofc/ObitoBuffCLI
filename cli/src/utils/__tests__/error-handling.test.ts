...
describe('error-handling', () => {
      ...
      expect(
        isFreeModeUnavailableError({
          ...
          responseBody: JSON.stringify({
            error: 'free_mode_unavailable',
            message: 'Obitobuff cannot be used from VPN traffic.',
          }),
        }),
      ).toBe(true)
    })
  ...
  describe('getObitobuffRateLimitErrorMessage', () => {
    test('returns the generic message for untyped 429 errors', () => {
      ...
      ).toBe(OBITOBUFF_RATE_LIMIT_MESSAGE)
    })
    ...
    test('returns null for non-429 status codes', () => {
      expect(getObitobuffRateLimitErrorMessage({ statusCode: 402 })).toBe(null)
      expect(getObitobuffRateLimitErrorMessage({ statusCode: 500 })).toBe(null)
    })

    test('returns null for string statusCode', () => {
      expect(getObitobuffRateLimitErrorMessage({ statusCode: '429' })).toBe(
        null,
      )
    })
    ...
    test('appends detail from agent-run output objects for untyped 429s', () => {
      expect(
        ...
        `${OBITOBUFF_RATE_LIMIT_MESSAGE} (Model is at capacity. Please try again later.)`,
      )
    })
  ...
  describe('isObitobuffProviderUsageError', () => {
    test('recognizes Obitobuff provider usage failures across upstream shapes', () => {
      for (const error of [
        {
          statusCode: 402,
          message: 'Payment required',
        ...
        expect(isObitobuffProviderUsageError(error)).toBe(true)
      }
    })
  ...
  describe('OBITOBUFF_RATE_LIMIT_MESSAGE', () => {
    test('encourages retry without mentioning credits or payment', () => {
      const message = OBITOBUFF_RATE_LIMIT_MESSAGE.toLowerCase()
  ...
  describe('OBITOBUFF_PROVIDER_USAGE_MESSAGE', () => {
    test('owns the refill without blaming the user account', () => {
      const message = OBITOBUFF_PROVIDER_USAGE_MESSAGE.toLowerCase()
      expect(message).toContain('obitobuff ran out of provider usage')
