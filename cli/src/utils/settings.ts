...
import { isLocalModelId } from '@codebuff/common/config/local-config'
import {
  OBITOBUFF_MODELS,
  isObitobuffModelId,
...
import { AGENT_MODES, IS_LOCAL_MODE } from './constants'
...
export interface Settings {
  mode?: AgentMode
  adsEnabled?: boolean
  /** Last model the user picked in the obitobuff model selector. Restored on
   *  next obitobuff launch so users land in the queue for their preferred
   *  model without re-picking. Persisted as the canonical model id. */
  obitobuffModel?: string
...
const validateSettings = (parsed: unknown): Settings => {
  ...
  // Validate obitobuffModel against the current picker catalog. Server support
  ...
  // In local mode, configured models are valid selections too.
  if (
    typeof obj.obitobuffModel === 'string' &&
    (isObitobuffModelId(obj.obitobuffModel) ||
      (IS_LOCAL_MODE && isLocalModelId(obj.obitobuffModel)))
  ) {
    settings.obitobuffModel = obj.obitobuffModel
  }

  // Steer off a model that has since been superseded (DeepSeek V4 Pro, MiniMax
  // M3, MiMo 2.5 → V4 Flash) on EVERY load, so each new obitobuff session starts
  ...
  const replacement = migrateSupersededObitobuffModelPreference(
    settings.obitobuffModel,
    OBITOBUFF_MODELS.map((model) => model.id),
  )
  if (replacement) settings.obitobuffModel = replacement
...
/**
 * Load the saved obitobuff model preference. Returns undefined if none is
 * saved yet — callers should fall back to DEFAULT_OBITOBUFF_MODEL_ID.
 */
export const loadObitobuffModelPreference = (): string | undefined => {
  return loadSettings().obitobuffModel
}

/**
 * Save an ordinary obitobuff picker preference so the next launch defaults to
 ...
 */
export const saveObitobuffModelPreference = (model: string): void => {
  if (!isObitobuffModelId(model) && !(IS_LOCAL_MODE && isLocalModelId(model))) {
    return
  }
  saveSettings({ obitobuffModel: model })
}
