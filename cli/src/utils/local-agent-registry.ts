...
import { IS_LOCAL_MODE, IS_OBITOBUFF, type AgentMode } from './constants'
import { getAgentIdForMode } from './obitobuff-agent-selection'
import { logger } from './logger'
import * as bundledAgentsModule from '../agents/bundled-agents.generated'

import type { AgentDefinition } from '@codebuff/common/templates/initial-agents-dir/types/agent-definition'
import type { ModelName } from '@codebuff/common/templates/initial-agents-dir/types/agent-definition'
...
export const loadLocalAgents = (
  ...
  const selectedObitobuffModel = IS_OBITOBUFF ? getSelectedObitobuffModel() : null
  const cacheKey = selectedObitobuffModel
    ? `${currentAgentMode ?? 'all'}:${selectedObitobuffModel}`
...
export const loadAgentDefinitions = (): AgentDefinition[] => {
  ...
  // Local mode: every agent runs on the locally-configured model, so requests
  // route to the configured provider instead of the backend. Overriding here
  // covers the root agent AND every spawned subagent.
  if (IS_LOCAL_MODE) {
    const localModel = getSelectedObitobuffModel()
    for (const def of definitions) {
      def.model = localModel as ModelName
    }
  }
