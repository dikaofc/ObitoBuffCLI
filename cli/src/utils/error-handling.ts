import { OBITOBUFF_PROVIDER_USAGE_ERROR_PATTERN } from '@codebuff/common/constants/obitobuff-errors'
...
import { getObitobuffGateCode } from '@codebuff/common/types/obitobuff-session'
...
export const getObitobuffRateLimitErrorMessage = (
  error: unknown,
): string | null => {
  const details = getCliApiErrorDetails(error)
  ...
  if (details.errorCode === 'free_mode_rate_limited') {
    ...
    return details.message ?? OBITOBUFF_RATE_LIMIT_MESSAGE
  }
  ...
  if (detail && !/^too many requests\.?$/i.test(detail)) {
    return `${OBITOBUFF_RATE_LIMIT_MESSAGE} (${detail})`
  }
  return OBITOBUFF_RATE_LIMIT_MESSAGE
}

/**
 * Provider billing failures are an operator problem in Obitobuff, not a reason
 ...
 */
export const isObitobuffProviderUsageError = (error: unknown): boolean => {
  const details = getCliApiErrorDetails(error)
  const message = details.message ?? extractErrorMessage(error, '')
  return (
    details.statusCode === 402 ||
    OBITOBUFF_PROVIDER_USAGE_ERROR_PATTERN.test(message)
  )
}
  ...
  return {
    ...
    countryBlockReason:
      typeof errorDetails.countryBlockReason === 'string'
        ? (errorDetails.countryBlockReason as ObitobuffCountryBlockReason)
        : undefined,
...
export const getFreeModeUnavailableErrorMessage = (
  ...
  if (block?.countryBlockReason === 'anonymous_network') {
    return `${IS_OBITOBUFF ? 'Obitobuff' : 'Free mode'} cannot be used from ${formatObitobuffHardBlockedPrivacySignals(
 ...
 * their statuses come from OBITOBUFF_GATE_CODES (the shared wire contract, see
 * docs/obitobuff-session-admission.md); the narrowing is deliberate —
 ...
 */
export type ObitobuffGateErrorKind = Exclude<
  ObitobuffGateCode,
  'session_limit_reached'
>

export const getObitobuffGateErrorKind = (
  error: unknown,
): ObitobuffGateErrorKind | null => {
  ...
  const code = getObitobuffGateCode({
    error: errorCode,
...
export const OBITOBUFF_RATE_LIMIT_MESSAGE =
  'Obitobuff is temporarily busy. Please try again in a moment.'

export const FREE_MODE_UNAVAILABLE_MESSAGE = IS_OBITOBUFF
  ? 'Obitobuff is not available in your country.'
