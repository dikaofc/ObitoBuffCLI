 ...
 * history and Obitobuff session-gate views replace the Chat surface.
 */
export const ChatRuntimeProvider = ({
  ...
  const obitobuffSession = useObitobuffSessionStore((state) => state.session)
  const sendBlocked = IS_OBITOBUFF && !holdsLiveObitobuffSlot(obitobuffSession)

  useEffect(() => {
    if (sendBlocked) {
      logger.info(
        {},
        '[chat-runtime] Obitobuff session over; holding queued messages until rejoin',
      )
    }
  }, [sendBlocked])
