...
import { ObitobuffSupersededScreen } from './components/obitobuff-superseded-screen'
import { LoginModal } from './components/login-modal'
import { ProjectPickerScreen } from './components/project-picker-screen'
import { ObitobuffLandingScreen } from './components/obitobuff-landing-screen'
import { useAuthQuery } from './hooks/use-auth-query'
import { useAuthState } from './hooks/use-auth-state'
import { useObitobuffSession } from './hooks/use-obitobuff-session'
...
export const App = ({
  ...
  // failures or a banned obitobuff session would otherwise replace the
  ...
  // Don't show login modal during network outages OR while retrying. Local
  // mode never needs an account, so no login gate may replace the chat.
  if (
    !IS_LOCAL_MODE &&
    requireAuth !== null &&
...
/**
 * Rendered only after auth is confirmed. Owns the obitobuff session gate
 * so `useObitobuffSession` runs exactly once per authed session (not before
 * we have a token).
 */
const AuthedSurface = (props: AuthedSurfaceProps) => {
  const { session, failure: sessionFailure } = useObitobuffSession()
...
const AuthedSurfaceRoutes = ({
  ...
  session: ReturnType<typeof useObitobuffSession>['session']
  sessionFailure: ReturnType<typeof useObitobuffSession>['failure']
}) => {
  ...
  if (IS_OBITOBUFF && !IS_LOCAL_MODE && session?.status === 'superseded') {
    return <ObitobuffSupersededScreen />
  }
  ...
  if (
    ...
    return <ObitobuffLandingScreen session={session} failure={sessionFailure} />
  }

  // Chat history renders inside AuthedSurface so the obitobuff session stays
