  ...
  () => {
    beforeAll(async () => {
      ...
      if (tmuxAvailable && sdkBuilt) {
        ...
        // Clear OBITOBUFF_MODE from the tmux global env. A previous obitobuff
        // build or `bun run dev:obitobuff` invocation in the same tmux server
        // can leave it set globally, which would make this test see the
        // obitobuff CLI variant (which has no `--agent` flag).
        await tmux(['set-environment', '-gu', 'OBITOBUFF_MODE']).catch(() => {})
      }
    })
