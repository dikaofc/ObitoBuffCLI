...
import { ObitobuffActiveSessionSummary } from './components/obitobuff-active-session-summary'
...
import { END_SESSION_MESSAGE, IS_LOCAL_MODE, IS_OBITOBUFF } from './utils/constants'
...
export const Chat = ({
  ...
  obitobuffSession: ObitobuffSessionResponse | null
}) => {
  ...
  // submits their first prompt ever (persisted in settings). Obitobuff only.
  const [showSuggestedPrompts, setShowSuggestedPrompts] = useState(
    () => IS_OBITOBUFF && !hasSubmittedFirstPrompt(),
  )
  ...
  const {
    ...
    // Local mode never shows or fetches ads: there is no ad-funded backend
    // to support, so keep the hook fully disabled (no network calls).
    enabled: !IS_LOCAL_MODE && (IS_OBITOBUFF || !hasSubscription),
    provider: 'gravity',
    inline: true,
    surface: 'cli_chat',
  ...
  const showInlineAds = (IS_OBITOBUFF && !IS_LOCAL_MODE) || getAdsEnabled()
  ...
  // output resumes.
  useEffect(() => {
    // Free-mode capacity deferrals come from the shared backend; local mode
    // talks only to configured providers, so there is nothing to listen for.
  ...
  // The panel store outlives this component and a Obitobuff session can end on
  ...
  const hasActiveObitobuffSession =
    IS_OBITOBUFF && obitobuffSession?.status === 'active'
  const isObitobuffSessionOver =
    IS_OBITOBUFF && obitobuffSession?.status === 'ended'
  const shouldShowStatusLine =
    !feedbackMode &&
    (hasStatusIndicatorContent ||
      shouldShowQueuePreview ||
      !isAtBottom ||
      hasActiveObitobuffSession)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {IS_OBITOBUFF && !IS_LOCAL_MODE && (
          <ObitobuffActiveSessionSummary session={obitobuffSession} />
        )}
      ...
      >
        {showOnboardingPrompts && !reviewMode && !isObitobuffSessionOver && (
          <SuggestedPrompts
        ...
        {shouldShowStatusLine && (
          <StatusBar
            ...
            // Local mode has no server session to end; the status bar button
            // is hidden entirely (nothing would happen if clicked).
            onEndSession={
              IS_LOCAL_MODE
                ? undefined
                : () => {
            ...
            obitobuffSession={obitobuffSession}
          />
        )}
        ...
        {reviewMode ? (
          ...
          />
        ) : isObitobuffSessionOver && !askUserState ? (
          <SessionEndedBanner
