...
function createLauncher(productConfig) {
  const {
    ...
    // Where the launcher learns about new versions and downloads binaries:
    // 'app' checks the npm registry for the version and downloads from the
    // Codebuff app release API; 'github' checks GitHub Releases for the
    // version and downloads release assets from github.com.
    releaseSource = 'app',
    // For releaseSource 'github': "owner/repo" whose releases host the
    // binaries (e.g. "dikaofc/ObitoBuffCLI"). Overridable at runtime with
    // OBITOBUFF_UPDATE_REPO / <PACKAGE>_UPDATE_REPO.
    githubRepo = null,
    // Tags are published with a 'v' prefix by the release workflow.
    githubTagPrefix = 'v',
  } = productConfig
  ...
  async function getLatestVersion() {
    try {
      const isGithub = releaseSource === 'github'
      // Same effective-repo resolution as stageBinary: an env override
      // (OBITOBUFF_UPDATE_REPO / <PACKAGE>_UPDATE_REPO) redirects BOTH the
      // version check and the download to the fork.
      const effectiveRepo =
        process.env[`${packageName.toUpperCase()}_UPDATE_REPO`] || githubRepo
  ...
  async function stageBinary(
    ...
    const effectiveRepo =
      process.env[`${packageName.toUpperCase()}_UPDATE_REPO`] || githubRepo
    const downloadUrl =
      releaseSource === 'github' && effectiveRepo
        ? `https://github.com/${effectiveRepo}/releases/download/${githubTagPrefix}${version}/${fileName}`
        : `${
