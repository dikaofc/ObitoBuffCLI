...
//
//   - "will run commands on your behalf" — codebuff/obitobuff main surface
//     header (authed + session ready)
//   - "Press ENTER to login" / "Open this URL" — login modal (no cached
//     creds — typical CI smoke)
//   - "Pick a model to start" — obitobuff model-picker landing screen
//   - "Free mode isn't available" — obitobuff country-block screen (CI
...
// fires after the renderer is up). The wasm-load rejection on obitobuff
...
// what tripped obitobuff 0.0.62 in the wild while a 5s window let CI pass.
