...
Tests that spawn a server child should race readiness against `proc.exited` and report the child's captured output (see `obitobuff-desktop/src/app/server.test.ts`). Polling a dead port surfaces only as "a hook timed out", which names neither the cause nor the process that failed.
...
**Refresh the baselines periodically, and act on the "stale" line.** Because growth never fails, a baseline nobody touches quietly becomes a floor far below reality — which is a guard that no longer guards. When this rot was first measured, `obitobuff-desktop` was recorded at 971 tests while actually running 1278, so **307 tests could have stopped running and CI would have stayed green** (393 across all suites). Re-record from a real CI run, not locally.
...
`bun --filter='*' run typecheck` scheduled `@codebuff/obitobuff-web` last: it did
...
matrix over *lanes* — `obitobuff-web` gets its own runner, `rest` takes the other
...
- **`--filter='*' --filter='!@codebuff/obitobuff-web'` does not exclude
...
table above recorded `obitobuff-desktop` at 130s, which was wrong: the job had
...
`obitobuff/web`'s `prepare:workspace` shells out to that same SDK build, and it
was the tail of the typecheck job — obitobuff/web finishes last, and spent its
...
still build normally. If you add a job that runs `obitobuff/web`'s typecheck or
build, either let it rebuild or set the flag *after* `build-sdk` — never before.

### Known remaining cost

`obitobuff-desktop` is the heaviest suite — ~54s on CI, about half of all test
...
The weekly `flake-hunt.yml` workflow runs this against `obitobuff-desktop` and
